...
#if defined(__VMKLNX__)
		old_pos = mousedev->packet.x;
#endif
		mousedev->packet.x =
			((value - dev->absmin[ABS_X]) * xres) / size;
		mousedev->packet.abs_event = 1;
#if defined(__VMKLNX__)
		vmk_InputEvent(EV_REL, REL_X, mousedev->packet.x - old_pos);
#endif
		break;
...
#if defined(__VMKLNX__)
		old_pos = mousedev->packet.y;
#endif
		mousedev->packet.y = yres -
			((value - dev->absmin[ABS_Y]) * yres) / size;
		mousedev->packet.abs_event = 1;
#if defined(__VMKLNX__)
		vmk_InputEvent(EV_REL, REL_Y, old_pos - mousedev->packet.y);
#endif
		break;
	}
}
...
#if defined(__VMKLNX__)
	vmk_InputEvent(EV_REL, code, value);
#endif
}
...
#if defined(__VMKLNX__)
	vmk_InputEvent(EV_KEY, code, value);
#endif
}
...
#if !defined(__VMKLNX__)
        /* Open device unconditionally. No one higher up to open it later. */
	if (mousedev_mix->open) {
#else
	{
#endif
