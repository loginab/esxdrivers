...
#if defined(__VMKLNX__)
struct vc_data;
extern u_char vmklnx_map_key(u_char value, u_char keytype);
...
{
#if defined(__VMKLNX__)
	printk(KERN_DEBUG "%s not implemented", __FUNCTION__);
#else
...
#if defined(__VMKLNX__)
	vmk_InputEvent(event_type, event_code, value);
#else
...
#if defined(__VMKLNX__)
static int kmap_major;
#define KEYMAP_DEV "keymap"

// mostly identical to do_kdsk_ioctl() in drivers/char/vt_ioctl.c
#define i (tmp.kb_index)
#define s (tmp.kb_table)
#define v (tmp.kb_value)
static int
kmap_ioctl(struct inode *unused1, struct file *unused2,
	   unsigned int cmd, unsigned long arg)
{
	struct kbentry *user_kbe = (struct kbentry *)arg;
...
static struct file_operations kmap_fops = {
 owner:THIS_MODULE,
 ioctl:kmap_ioctl
};
#endif // __VMKLNX__
...
#if defined(__VMKLNX__)
	kmap_major = register_chrdev(0, KEYMAP_DEV, &kmap_fops);
#endif
	tasklet_enable(&keyboard_tasklet);
	tasklet_schedule(&keyboard_tasklet);
