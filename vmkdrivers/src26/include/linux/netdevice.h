...
#if defined(__VMKLNX__)
/* irq shield is only intended to protect napi->poll_list */
#define napi_complete(n) __napi_complete(n)
#else /* !defined(__VMKLNX__) */
 ...
 */
        vmk_PktCompletionData   genCount;          /* gen counter for packet completion. */
...
#define HAVE_TX_TIMEOUT
	 ...
	 */
        ...
        int                     link_speed;
        vmk_UplinkLinkState     link_state;
        int                     full_duplex;      
        ...
        vmk_UplinkWatchdogPanicModState  watchdog_timeohit_panic;   /* Panic mod */
#define NETDEV_TICKS_PER_HOUR  HZ * 3600
	...
	int			useDriverNamingDevice;
        ...
        vmk_Worldlet            napi_worldlet;
        vmk_Worldlet            default_worldlet;
        struct napi_wdt_priv    napi_wdt_priv;
...
#else
extern void             vmklnx_free_netdev(struct vmklnx_pm_skb_cache *pmCache,
                                           struct net_device *dev);
/**
...
{
        vmklnx_free_netdev(&vmklnx_pm_skb_cache, dev);
}
#endif
...
#if defined(__VMKLNX__)
static inline void __netif_schedule(struct netdev_queue *queue)
{
   if (!test_and_set_bit(__QUEUE_STATE_SCHED, &queue->state)) {
      unsigned long flags;
      int cpu = smp_processor_id();
      local_irq_save(flags);
      queue->next_sched = softnet_data[cpu].output_queue;
      softnet_data[cpu].output_queue = queue;
...
#if defined(__VMKLNX__)
/* irq shield is only intended to protect napi->poll_list */
#define netif_rx_complete(dev, napi) __netif_rx_complete(dev, napi)
#else /* !defined(__VMKLNX__) */
/**                                          
...
#if defined(__VMKLNX__)
/*
 * vmklinux26 doesn't support old napi anymore so let's drop this code.
 */
#else /* !defined(__VMKLNX__) */
