...
static inline void dma_chan_get(struct dma_chan *chan)
{
	...
	/* FIXME: skip per_cpu refcount bookkeeping */
	else {
...
struct dma_device {
	...
	struct dma_async_tx_descriptor *(*device_prep_dma_interrupt)(
        ...
        dma_cookie_t (* device_memcpy_mach_buf_to_buf)(struct dma_chan *chan,
        ...
        /* FIXME: this is duplicated with device_is_tx_complete() */
...
struct vmk_ioatdevice {
  void (*client_register)(struct dma_client *client);
  void (*client_unregister)(struct dma_client *client);
  void (*client_chan_request)(struct dma_client *client);//, unsigned int number);
  void (*client_chan_remove)(struct dma_chan *chan);
  uint8_t (*dca_get_tag)(int cpu);
  int (*dca_add_requester)(struct device *dev);
  int (*dca_remove_requester)(struct device *dev);
  struct dma_device *vmk_dmadevice;
};

int vmklnx_IoatRegister(struct vmk_ioatdevice *dev);
void vmklnx_IoatCleanup(void);
...
void dma_clients_notify_removed(struct dma_chan *chan);
