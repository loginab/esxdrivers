 ...
 *
 *  SEE ALSO:
 *  spin_unlock, spin_trylock_bh, spin_trylock_irq, spin_trylock_irqsave
 *
 *  RETURN VALUE:
 ...
 *
 *  SEE ALSO:
 *  spin_unlock_bh, spin_trylock_bh, spin_lock_irqsave
 *
 *  RETURN VALUE:
 *  None
 *
...
/**
 *  spin_trylock_bh - Attempt to acquire a bottom-half spinlock without spinning
 *  @lock: lock to acquire
 *
 *  Attempts to acquire a spinlock that is otherwise acquired and released using 
 *  spin_lock_bh and spin_unlock_bh.  spin_trylock_bh does not spin if the lock
 *  is already held.  If the lock-acquire is successful, spin_trylock_bh 
 *  disables bottom-half execution.  Do not use this variant if the lock could
 ...
 *
 *  SEE ALSO:
 *  spin_unlock_bh, spin_trylock_irq, spin_trylock_irqsave
 *
 *  RETURN VALUE:
 *  TRUE if lock acquisition succeeded, FALSE if the lock was already held
 *
 */
/* _VMKLNX_CODECHECK_: spin_trylock_bh */
...
{
   return spin_is_locked(lock) && (lock->raw_lock.cpu == vmk_GetPCPUNum());
}
#endif /* defined(__VMKLNX__) */
