				   ...
				   int off, int copy)
{
	if (skb->ip_summed == CHECKSUM_NONE) {
		int err = 0;
                unsigned int csum =
#if defined(__VMKLNX__)
                   csum_and_copy_from_user((const unsigned char __user *)from,
...
{
/*
 * TODO:<lzhou> in_softirq() is not implemented in vmklinux26
 * Since no one uses gfp_any() today, just always return GFP_ATOMIC for now
 * as the workaround.
 */
#if defined(__VMKLNX__) 
	return GFP_ATOMIC;
