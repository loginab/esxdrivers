...
#define LINUX_VERSION_CODE 0x020618
#define __NO_TPA__ 1
#define KERN_CONT KERN_INFO
/* For driver stability, disable IPv6 checksum and TSO. */
#define __VMKLNX_NO_IPV6_CSUM__		1
#undef NETIF_F_TSO6
...
#define DRV_MODULE_RELDATE	"$DateTime: 2008/08/25 07:40:44 $"
...
#ifndef BNX2X_UPSTREAM /* ! BNX2X_UPSTREAM */
#if (LINUX_VERSION_CODE >= 0x020600) /* ! BNX2X_UPSTREAM */
MODULE_INFO(cvs_version, "$Revision: #41 $");
#endif
#endif
...
static int use_inta;
#else
static int use_inta = 1;
#endif
static int poll;
static int debug;
static int load_count[3]; /* 0-common, 1-port0, 2-port1 */
#if defined(__VMKLNX__) && defined(__VMKNETDDI_QUEUEOPS__)
static int use_multi = 1;
#else
static int use_multi;
#endif
static int enable_wol = 1;

module_param(enable_wol, int, 0);
MODULE_PARM_DESC(enable_wol, "enable WOL feature support in driver.");
#if (LINUX_VERSION_CODE >= 0x020600) /* BNX2X_UPSTREAM */
#if !defined(__NO_TPA__)
...
#elif defined(__VMKLNX__) && defined(__VMKNETDDI_QUEUEOPS__)
module_param(use_multi, int, 0);
MODULE_PARM_DESC(use_multi, " RX queue number for supporting NetQueue");
#endif
#endif
...
#if defined(__VMKLNX__) && defined(__VMKLNX_NO_IPV6_CSUM__)
...
*/
	...
	if (unlikely(__netif_subqueue_stopped(bp->dev, fp->index))) {


		if (__netif_subqueue_stopped(bp->dev, fp->index) &&
		    (bp->state == BNX2X_STATE_OPEN) &&
		    (bnx2x_tx_avail(fp) >= MAX_SKB_FRAGS + 3))
			netif_wake_subqueue(bp->dev, fp->index);
			   ...
			   union eth_rx_cqe *rr_cqe)
{
	...
	case (RAMROD_CMD_ID_ETH_HALT | BNX2X_STATE_CLOSING_WAIT4_HALT):
		...
		/* With netqueue enabled, all client HALT command will
		 * come from 0 queue, so need to update the cid queue
		 */
		bnx2x_fp(bp, cid, state) = BNX2X_FP_STATE_HALTED;
#else
...
#if defined(__VMKLNX__) && defined(__VMKNETDDI_QUEUEOPS__)
			/* hardware will report some 0 length packets
			 * when it is being closed */
			if (len == 0 || bp->state != BNX2X_STATE_OPEN)
				goto reuse_rx;
#endif
...
#ifdef CONFIG_PCI_MSI /* BNX2X_UPSTREAM */
#if (LINUX_VERSION_CODE < 0x20613) && !defined(__USE_COMPAT_LAYER_2_6_18_PLUS__)
...
static inline u16 bnx2x_update_dsb_idx(struct bnx2x *bp)
{
	...
	barrier(); /* status block is written to by the chip */
#if defined( __VMKLNX__)
	/* Default status block not initialized */
	VMK_ASSERT(def_sb);
	if (!def_sb) {
		printk(KERN_WARNING
			"bnx2x: Default status block not initialized!\n");
		return 0;
	}
#endif /* __VMKLNX__ */
...
#if defined(INIT_DELAYED_WORK_DEFERRABLE) || defined(INIT_WORK_NAR) || defined(__VMKLNX__) /* BNX2X_UPSTREAM */
...
#if defined(__VMKLNX__) && defined(__VMKNETDDI_QUEUEOPS__)
		/* enable the queue 0 as the default queue */
...
#if defined(__VMKLNX__)
	/* Workaround (VMKLNX and COS):
	 * 10 msecs is shorter for the 57711 blade build-in NIC.
	 * Fix it to 100 msecs.
	 */
	delay = 100;
#endif
...
#if defined(__VMKLNX__) && defined(__VMKNETDDI_QUEUEOPS__)
		/* Workaround: sleep 1ms before return for both 57710 and 57711 */
		msleep(1);
		return 0;
#endif
...
#if defined(__VMKLNX__) && defined(__VMKNETDDI_QUEUEOPS__)
	/* Workaround for PR347954.
	 * Disable NetQueue when Flex10 module is connected.
	 */
	if (IS_E1HMF(bp))
		use_multi = 0;
#endif
...
#if defined(__VMKLNX__) && defined(__VMKNETDDI_QUEUEOPS__)
			/* Limit the number of queues to maximum number
			 * of physical cpus
			 */
			bp->num_queues = min_t(u32, bp->num_queues,
						num_online_cpus());
#endif
...
#ifdef CONFIG_NETDEVICES_MULTIQUEUE
	/* Notify the real tx queue number */
...
static void __devinit bnx2x_get_common_hwinfo(struct bnx2x *bp)
{
	...
	if (!enable_wol) {
		/* no WOL capability for module load parameter
		 * 'enable_wol' is not set.
		 */
		bp->flags |= NO_WOL_FLAG;
	} else if (BP_E1HVN(bp) == 0) {
...
#ifdef BNX2X_RXTX_QUEUE_STATS
	info->n_stats += BNX2X_QUEUE_STAT_CNT * bp->num_queues;
#endif
...
#ifdef BNX2X_RXTX_QUEUE_STATS
		buf += j * ETH_GSTRING_LEN;
		for (i = 0; i < bp->num_queues; i++) {
			sprintf(buf, "Rx/Txqueue[%u]->poll_times", i);
			buf += ETH_GSTRING_LEN;
			sprintf(buf, "\tTxqueue[%u]->tx_packets", i);
			buf += ETH_GSTRING_LEN;
			sprintf(buf, "\tTxqueue[%u]->tx_free_packets", i);
			buf += ETH_GSTRING_LEN;
			sprintf(buf, "\tTxqueue[%u]->tx_bytes", i);
			buf += ETH_GSTRING_LEN;
			sprintf(buf, "\tTxqueue[%u]->tx_int_calls", i);
			buf += ETH_GSTRING_LEN;
			sprintf(buf, "\tRxqueue[%u]->rx_packets", i);
			buf += ETH_GSTRING_LEN;
			sprintf(buf, "\tRxqueue[%u]->rx_bytes", i);
			buf += ETH_GSTRING_LEN;
			sprintf(buf, "\tRxqueue[%u]->rx_calls", i);
			buf += ETH_GSTRING_LEN;
		}
#endif
		break;
...
#ifdef BNX2X_RXTX_QUEUE_STATS
	num_stats += BNX2X_QUEUE_STAT_CNT * bp->num_queues;
#endif
	return num_stats;
}
...
#ifdef BNX2X_RXTX_QUEUE_STATS
	for (i = 0; i < bp->num_queues; i++) {
		buf[j++] = (u64)bnx2x_fp(bp, i, poll_times);
		buf[j++] = (u64)bnx2x_fp(bp, i, tx_pkt);
		buf[j++] = (u64)bnx2x_fp(bp, i, tx_free_pkt);
		buf[j++] = (u64)bnx2x_fp(bp, i, tx_bytes);
		buf[j++] = (u64)bnx2x_fp(bp, i, tx_calls);
		buf[j++] = (u64)bnx2x_fp(bp, i, rx_pkt);
		buf[j++] = (u64)bnx2x_fp(bp, i, rx_bytes);
		buf[j++] = (u64)bnx2x_fp(bp, i, rx_calls);
	}	
#endif
}
...
#endif
#if defined(__VMKLNX__) && defined(__VMKNETDDI_QUEUEOPS__)
	VMK_ASSERT(skb->queue_mapping < bp->num_queues);
...
#ifdef CONFIG_NETDEVICES_MULTIQUEUE
		netif_stop_subqueue(dev, fp_index);
#else
...
#ifdef CONFIG_NETDEVICES_MULTIQUEUE
		netif_stop_subqueue(dev, fp->index);
#else
		netif_stop_queue(dev);
#endif
		bp->eth_stats.driver_xoff++;
...
#ifdef CONFIG_NETDEVICES_MULTIQUEUE
			netif_wake_subqueue(dev, fp->index);
#else
...
#if defined(__VMKLNX__) && defined(__VMKLNX_NO_IPV6_CSUM__)
	/* Only enable IPv4 checksum offload */
	dev->features |= NETIF_F_IP_CSUM;
#else
	/* Enable both IPv4 and IPv6 checksum offload */
...
#ifdef CONFIG_NETDEVICES_MULTIQUEUE
	dev = alloc_etherdev_mq(sizeof(*bp), MAX_QUEUES_NUM);
#else
...
static int bnx2x_get_queue_count(vmknetddi_queueop_get_queue_count_args_t *args)
{
	...
	if (args->type == VMKNETDDI_QUEUEOPS_QUEUE_TYPE_RX) {
		args->count = max_t(u16, bp->num_queues - 1, 0);
		return VMKNETDDI_QUEUEOPS_OK;
	}
...
static int
bnx2x_alloc_rx_queue(struct net_device *netdev,
		      		...
		      		u16 *queue_mapping)	
{
	...
	if (bp->n_tx_queues_allocated >= bp->num_queues) {
		return VMKNETDDI_QUEUEOPS_ERR;
        }
	for_each_nondefault_queue(bp, i) {
		struct bnx2x_fastpath *fp = &bp->fp[i];
		if (!fp->tx_queue_allocated) {
			fp->tx_queue_allocated = TRUE;
...
static int
bnx2x_free_tx_queue(struct net_device *netdev,
...
bnx2x_apply_rx_filter(vmknetddi_queueop_apply_rx_filter_args_t *args)
{
	...
	if (fp->rx_queue_active || !fp->rx_queue_allocated) {
		return VMKNETDDI_QUEUEOPS_ERR;
	}
...
static int
bnx2x_get_queue_stats(vmknetddi_queueop_get_stats_args_t *args)
{
	return VMKNETDDI_QUEUEOPS_ERR;
}

static int
bnx2x_get_netqueue_version(vmknetddi_queueop_get_version_args_t *args)
{
	return vmknetddi_queueops_version(args);
}

static int
bnx2x_netqueue_ops(vmknetddi_queueops_op_t op, void *args)
{
