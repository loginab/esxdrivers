...
#define DRV_VERSION "8.0.3.1" DRV_NAPI DRV_DEBUG DRV_HW_PERF
...
/*
 * We won't use this RX handler, so remove prototype to avoid warning.
 */
...
#if defined(__VMKLNX__)
static inline void e1000_errata24_free_held_buffers(struct e1000_adapter *);
static inline void e1000_errata24_free_badskbs(struct e1000_adapter *adapter);
#endif // __VMKLNX__
...
#if defined(__VMKLNX__)
        /*
         * On TX timeout, leave one second before the virtual E1000
         * also timeout. See PR161852.
         */
...
#else /* defined(__VMKLNX__) */
	netif_napi_add(adapter->netdev, &adapter->napi, e1000_poll, 64);
#endif /* !defined(__VMKLNX__) */
...
#if defined(__VMKLNX__)
	e1000_errata24_free_held_buffers(adapter);
	e1000_errata24_free_badskbs(adapter);
#endif
...
#if defined(__VMKLNX__)
static inline void
e1000_errata24_free_held_buffers(struct e1000_adapter *adapter)
{
	int i;
	struct pci_dev *pdev = adapter->pdev;
	for (i = 0; i < adapter->errata24_badbuffer_idx; i++) {
		unsigned int size = adapter->errata24_badbuffers[i].size;
		dma_addr_t dma = adapter->errata24_badbuffers[i].dma;
		void *desc = adapter->errata24_badbuffers[i].desc;
		pci_free_consistent(pdev, size, desc, dma);
	}
	adapter->errata24_badbuffer_idx = 0;
}

static inline bool
e1000_errata24_hold_badbuffer(struct e1000_adapter *adapter,
                              void *desc, dma_addr_t dma, unsigned int size)
{
	unsigned int idx = adapter->errata24_badbuffer_idx;

	if (idx >= MAX_ERRATA24_BADBUFFERS) {
		printk("Warning: already holding on to %d"
		       "errata24 bad buffers\n", MAX_ERRATA24_BADBUFFERS);
		return FALSE;
	}

	adapter->errata24_badbuffers[idx].desc = desc;
	adapter->errata24_badbuffers[idx].dma = dma;
	adapter->errata24_badbuffers[idx].size = size;

	adapter->errata24_badbuffer_idx++;
	return TRUE;
}

bool
e1000_errata24_fix(struct e1000_adapter *adapter,
                   void **desc,
                   dma_addr_t *dma,
                   unsigned int size)
{
	...
	if (!e1000_check_64k_bound(adapter, *desc, size)) {
		void *olddesc = *desc;
		dma_addr_t olddma = *dma;

		/* try to allocate a new buffer */
		*desc = pci_alloc_consistent(pdev, size, dma);
		if (!*desc) {
			*desc = olddesc;
			*dma = olddma;
			return FALSE;
		}
		/* check boundary condition for new buffer */
		if (!e1000_check_64k_bound(adapter, *desc, size)) {
			if (!e1000_errata24_hold_badbuffer(adapter, *desc,
							   *dma, size)) {
				pci_free_consistent(pdev, size, *desc, *dma);
			}
			*desc = olddesc;
			*dma = olddma;
			...
			/* hang onto problematic buffer */
			if (!e1000_errata24_hold_badbuffer(adapter, olddesc,
							   olddma, size)) {
...
/*
 * e1000_errata24_hold_badskb
 *
 * to manage errata 24 (crossing 64k boundaries), we save
 * bad skb's.  in the receive side code, when we allocate
 * skb's, we review the boundaries to decide whether its
 * bad.  if it is, we save it here.
 *
 * the code originally free'd back the packet.  in a quiescent
 * system, the same packet would be reviewed again shortly
 * since we manage returned packet LIFO.  since that same
 * bad packet would reappear, the packet behind it would get
 * used, and the bad packet would again get free'd back to
 * the LIFO.  if TWO bad packets appeared, we'd give up, AND
 * return both packets to the LIFO.  the next request would
 * unfornatly show up both bad packets, and we'd give up again.
 *
 */

static inline bool
e1000_errata24_hold_badskb(struct e1000_adapter *adapter, struct sk_buff *skb)
{
	unsigned int idx = adapter->errata24_badskb_idx;
	if (idx >= MAX_ERRATA24_BADSKBS) {
		printk("Warning: (errata24) already holding on to %d"
		       "bad skb buffers\n", MAX_ERRATA24_BADSKBS);
		return FALSE;
	}

	adapter->errata24_badskbs[idx] = skb;
	adapter->errata24_badskb_idx++;
	return TRUE;
}

/*
 * e1000_errata24_free_badskbs
 *
 */

static inline void
e1000_errata24_free_badskbs(struct e1000_adapter *adapter)
{
	int i;
	for (i = 0; i < adapter->errata24_badskb_idx; ++i) {
		dev_kfree_skb(adapter->errata24_badskbs[i]);
	}
	adapter->errata24_badskb_idx = 0;
}
#endif // __VMKLNX__
                                    ...
                                    struct e1000_tx_ring *tx_ring)
{
	...
	tx_ring->desc = pci_alloc_consistent(pdev, tx_ring->size,
	                                     &tx_ring->dma);
	if (!tx_ring->desc) {
setup_tx_desc_die: __maybe_unused
...
#if defined(__VMKLNX__)
	if (!e1000_errata24_fix(adapter, &tx_ring->desc, &tx_ring->dma, tx_ring->size)) {
		pci_free_consistent(pdev, tx_ring->size, tx_ring->desc, tx_ring->dma);
		DPRINTK(PROBE, ERR, "Unable to allocate aligned memory "
                                    ...
                                    struct e1000_rx_ring *rx_ring)
{
	...
	if (!rx_ring->desc) {
		DPRINTK(PROBE, ERR,
		"Unable to allocate memory for the receive descriptor ring\n");
setup_rx_desc_die: __maybe_unused
		vfree(rx_ring->buffer_info);
		return -ENOMEM;
	}

#if defined(__VMKLNX__)
	if (!e1000_errata24_fix(adapter, &rx_ring->desc, &rx_ring->dma, rx_ring->size)) {
...
#if defined(__VMKLNX__)
                        /*
                         * Workaround since we don't have the
                         * cancel_work functions.
                         *
                         * Can be removed when PR256765 is fixed.
                         */
                        dev_hold(netdev);
#endif /* defined(__VMKLNX__) */
...
#else /* !defined(__VMKLNX__) */
	/* 
 	 * We try to keep the adapter in bulk_latency. Due to bursty traffic 
         * we shift from bulk_latency to low_latency. This casues 
	 * high interrupt rate. We try to keep interrupt rate low for at least 
	 * 3 interrupts before deciding to move to low_latency. 
	 */

        if (adapter->rx_itr > adapter->tx_itr) {                                          
           current_itr = adapter->rx_itr;                                              
           if(adapter->itr < 7000) {                                                 
              adapter->rx_itr = bulk_latency;                                   
           }                                                                         
        } else {                                                                          
           current_itr = adapter->tx_itr;                                              
           if(adapter->itr < 7000) {                                                 
               adapter->tx_itr = bulk_latency;                                   
           }                                                                          
        } 
#endif /* !defined(__VMKLNX__) */
	switch (current_itr) {
...
#else /* !defined(__VMKLNX__) */
		             min(adapter->itr + (new_itr >> 4), new_itr) :
#endif /* !defined(__VMKLNX__) */
		             new_itr;
		adapter->itr = new_itr;
...
#if defined(__VMKLNX__)
                /*
                 * If there is more frags to come, then we are not
                 * terminating the buffer and don't need the
                 * workaround.
                 */
		if (unlikely(adapter->pcix_82544 && !nr_frags &&
                                 ...
                                 struct e1000_tx_ring *tx_ring, int size)
{
	...
	/* A reprieve! */
#ifdef CONFIG_NETDEVICES_MULTIQUEUE
	/* Only one subqueue, which is our only queue. */
	netif_start_subqueue(netdev, 0);
#else
...
#ifdef CONFIG_NETDEVICES_MULTIQUEUE
		if (__netif_subqueue_stopped(netdev, 0) &&
...
#if !defined(__VMKLNX__)
/*
 * We don't use this RX handler since it assumes packet split.
 */
#ifdef CONFIG_E1000_NAPI
/**
                                         ...
                                         int cleaned_count)
{
	...
	while (cleaned_count--) {
		...
		if (!e1000_check_64k_bound(adapter, skb->data, bufsz)) {
			...
			if (!skb) {
#if defined(__VMKLNX__)
				e1000_errata24_hold_badskb(adapter, oldskb);
#else // __VMKLNX__
			...
			if (!e1000_check_64k_bound(adapter, skb->data, bufsz)) {
				/* give up */
#if defined(__VMKLNX__)
				e1000_errata24_hold_badskb(adapter, skb);
				e1000_errata24_hold_badskb(adapter, oldskb);
#else // __VMKLNX__
				dev_kfree_skb(skb);
				dev_kfree_skb(oldskb);
