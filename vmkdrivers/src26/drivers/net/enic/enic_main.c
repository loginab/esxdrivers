 ...
 *
 */
#ident "$Id: enic_main.c 23540 2009-02-06 04:57:14Z vkolluri $"
...
static int enic_set_mac_addr(struct net_device *netdev, char *addr)
{
	...
	/* For ESX VFs, we ignore MAC addr check.
	 * We allow zero MAC addr for VFs.
	 */
                                                                                
	if (enic->pdev->device != PCI_DEVICE_ID_CISCO_PALO_ENET_PT) {
...
#if defined(__VMKLNX__)
	/* For ESX VFs, we ignore Rx filters from vmkernel.
	 * Rx filters will get installed on VF when VM connects.
	 */
...
{
	union enic_ioctl *io = (union enic_ioctl *)ifr;
	struct enic *enic = netdev_priv(netdev);
	u64 *a0, *a1, a0_bounce;
	int bounce = 0;
	...
	case ENIC_DEVCMD:
		...
		/* Only allow a subset of devcmds to be callable
		 * using devcmd pass-thru ioctl.
		 */

		switch (io->devcmd.cmd) {
		
		case CMD_MCPU_FW_INFO:
		case CMD_INIT_PROV_INFO:
		case CMD_STATS_DUMP:
		case CMD_STATS_DUMP_ALL:
			
			/* These require a DMA'able bounce buffer
			 * allocated from enic's heap.  Well copy
			 * out of this bounce buffer into supplied
			 * buffer when devcmd is finished.
			 */
		...
		if (bounce) {
			bounce_buf = pci_alloc_consistent(enic->pdev,
				*a1, &a0_bounce);
			if (!bounce_buf)
				return -ENOMEM;
			memcpy(bounce_buf, (void *)*a0, *a1);
			a0 = &a0_bounce;
		}

		spin_lock(&enic->devcmd_lock);
		ret = vnic_dev_cmd(enic->vdev, io->devcmd.cmd,
			a0, a1, io->devcmd.wait);
		spin_unlock(&enic->devcmd_lock);

		if (bounce) {
			a0 = &io->devcmd.a0;
			memcpy((void *)*a0, bounce_buf, *a1);
			pci_free_consistent(enic->pdev, *a1,
				bounce_buf, a0_bounce);
		}
...
#if !defined(__VMKLNX__)
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 24))
static int __enic_poll(struct net_device *netdev, int *budget)
{
	struct enic *enic = netdev_priv(netdev);
	struct napi_struct *napi = &enic->napi;
	unsigned int work_done;
	unsigned int work_to_do = min(*budget, netdev->quota);
...
static int enic_open(struct net_device *netdev)
{
	...
	/* Do not add station addr for VF.  The station
	 * addr will be set after or during the call to
	 * init_prov_info
	 */

	if (enic->pdev->device == PCI_DEVICE_ID_CISCO_PALO_ENET) {
#endif
	...
	const struct pci_device_id *ent)
{
	...
	/* Do not call dev_init for a VF.  For a VF, 
	 * init_prov_info will be called later by an
	 * upper layer via devcmd ioctl
	 */

	if (pdev->device == PCI_DEVICE_ID_CISCO_PALO_ENET) {
#endif
...
#if 0 || defined(__VMKLNX__)

	/* For VFs, use MSI interrupt mode rather than MSI-X
	 * due to limited number of interrupt vectors provided
	 * by ESX.  The VF running MSI mode uses one interrupt
	 * vector, whereas the same VF running in MSI-X mode
	 * uses 4 interrupt vectors.
	 */

	if (pdev->device == PCI_DEVICE_ID_CISCO_PALO_ENET_PT) {
	...
	/* For VFs, override the normal netdev naming and hardcode the
	 * netdev name as vf_vmnic%d.  This is necessary to avoid
	 * etherswitch from binding to vmnic0 when visor comes up,
	 * when the vmnic0 is a VF.  We want etherswitch binding to
	 * the first non-VF vmnic.
	 */

	if (pdev->device == PCI_DEVICE_ID_CISCO_PALO_ENET_PT) {
		static unsigned int vf = 0;
		netdev->useDriverNamingDevice = 1;
