/* Portions Copyright (c) VMware, Inc. 2007-2009. All Rights Reserved. */
/*
...
{
#if defined(__VMKNLX__)
	if (!(tp->tg3_flags & TG3_FLAG_TAGGED_STATUS) && tp->hw_status &&
...
static void tg3_timer(unsigned long __opaque)
{
	 ...
	 * beter agrees with it. See PR 88299 for more details, but there 
	...
	if (netif_carrier_ok(tp->dev)) {
		...
		if (!(rx_mode & RX_MODE_PROMISC) && (rx_mode != tp->rx_mode)) {
			static int rx_mode_reset_counter = 0;
			/* 
			 ...
			 */
			if (rx_mode_reset_counter < 200) {
				printk(KERN_INFO PFX "%s: rx_mode "
...
#if defined(__VMKLNX__)
			/* 10 seconds timeout for vmkernel. see pr141646 */
			tg3_write_mem(tp, NIC_SRAM_FW_CMD_DATA_MBOX, 10);
#else
			/* 5 seconds timeout */
...
#if defined(__VMKLNX__)
	/*
	 * See comment in tg3_init_one().
	 */
	dev->features &= ~(NETIF_F_TSO | NETIF_F_TSO6 | NETIF_F_TSO_ECN);
#endif /* defined(__VMKLNX__) */
...
static int __devinit tg3_get_invariants(struct tg3 *tp)
{
	...
	if (GET_ASIC_REV(tp->pci_chip_rev_id) != ASIC_REV_5705 &&
		...
		tp->tg3_flags2 |= TG3_FLG2_JUMBO_CAPABLE;
                printk(KERN_INFO "JF capable; asic_rev: 0x%x\n",
                       GET_ASIC_REV(tp->pci_chip_rev_id));
        } else {
                printk(KERN_INFO "no JF; asic_rev: 0x%x\n",
                       GET_ASIC_REV(tp->pci_chip_rev_id));
        }
...
#if defined(__VMKLNX__)
	/*
	 * Tg3 nics have a long history of problems with TSO frames and
	 * their implementation don't provide any significant boost anyway,
	 * so don't advertise them as TSO-capable.
	 */
	dev->features &= ~(NETIF_F_TSO | NETIF_F_TSO6 | NETIF_F_TSO_ECN);
#endif /* defined(__VMKLNX__) */
