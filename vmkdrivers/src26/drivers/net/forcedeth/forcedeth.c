...
#if defined (__VMKLNX__)
/* Disabling the netif_addr_lock/unlock as they are not yet implemented.
 * Please remove this if/when they are implemented.
 */
#ifndef netif_addr_lock
#define netif_addr_lock(x)
#endif

#ifndef netif_addr_unlock
#define netif_addr_unlock(x)
#endif
...
#if defined (__VMKLNX__)
#define NVREG_IRQ_FATAL_ERROR		0x0200
#endif /* defined (__VMKLNX__) */
...
#if defined (__VMKLNX__)
// Limiting to 4k as 16k is too much and puts strain on driver 
// resources.
#define RING_MAX_DESC_VER_2_3	4096
#else
...
#if defined (__VMKLNX__)
enum spcl_dev_hndl_t {
	NV_DO_SPECIAL_DEV_HANDLING,
};
#endif /* defined (__VMKLNX__) */
...
struct fe_priv {
	...
	int recover_error;
#if defined (__VMKLNX__)
	/* fields related to 'special device handling'.
	 * PR 381001 and 371090.
	 */
	int special_dev_handling;
	spinlock_t special_dev_handling_lock;			/* Lock to keep out nv_do_nic_poll() */
	struct work_struct special_dev_handling_task; /* actual task to do the reinit */
#endif /* defined (__VMKLNX__) */
...
#if defined (__VMKLNX__)
/* Workaround for pr 290182 */
static void nv_hard_reset(struct pci_dev *pci_dev, int delay)
{
	int err;

	pci_save_state(pci_dev);
	err=pci_set_power_state(pci_dev, PCI_D3cold);
	if (err) {
		printk("nv_hard_reset: Error in setting D3Cold state - 0x%x \n", err);
	}

	// if delay just do udelay for 200 us
	if (delay)
		udelay(200);
	else
		msleep(5);
	...
	if (err) {
		printk("nv_hard_reset: Error in setting D0 state back 0x%x \n", err);
	}
...
static void nv_txrx_reset(struct net_device *dev)
{
    ...
    pci_push(base);
#else
    /* Enabling this bit causes the RX logic to hang bad enough to require
     * a power cycle to clear it up again.
     * bug 290182
     */
#endif /* !defined (__VMKLNX__) */
}
...
#if defined (__VMKLNX__)
	/* prevent deadlock with nv_do_nic_poll() -- see pr 321589 */
	if (!(spin_trylock_irqsave(&np->lock, flags)))
...
static void nv_tx_timeout(struct net_device *dev)
{
...
#if defined (__VMKLNX__)
	spin_lock_irq(&np->lock);
	if (test_bit(NV_DO_SPECIAL_DEV_HANDLING, &np->special_dev_handling)) {
		spin_unlock_irq(&np->lock);
		return;
	}
	spin_unlock_irq(&np->lock);
	...
	/*
	 * Disable register dump on Tx timeout. Register dump makes the system
	 * freezing for a few seconds, and causes consecutive watchdog timeout
	 */
#if !defined (__VMKLNX__)
	{
		int i;
	...
	if (!nv_optimized(np)) {
		if (le32_to_cpu(np->get_tx.orig->flaglen) & NV_TX_VALID)
			printk(KERN_INFO "nv_tx_timeout: possible tx hang!!\n");
	} else {
		if (le32_to_cpu(np->get_tx.ex->flaglen) & NV_TX_VALID)
			printk(KERN_INFO "nv_tx_timeout: possible tx hang!!\n");
	}
...
{
	if (!(test_and_set_bit(NV_DO_SPECIAL_DEV_HANDLING, &np->special_dev_handling))) {
		np->nic_poll_irq = np->irqmask;
		schedule_work(&np->special_dev_handling_task);
	}
}
#endif /* defined(__VMKLNX__) */
...
static int nv_change_mtu(struct net_device *dev, int new_mtu)
{
	...
	if (netif_running(dev)) {
		...
		if (np->driver_data & DEV_HAS_POWER_CNTRL) {
			...
			set_bit(NV_DO_SPECIAL_DEV_HANDLING, &np->special_dev_handling);
			nv_do_special_dev_handling(np);
		}
#endif /* defined(__VMKLNX__) */
	}
	return 0;
...
#if defined(__VMKLNX__)
                    /*
                     * Need this check to make sure that tx pause doesnt
                     * get turned on even if it is disabled. PR 258204
                     */
                    if (np->pause_flags & NV_PAUSEFRAME_TX_REQ)
#endif
					pause_flags |= NV_PAUSEFRAME_TX_ENABLE;
...
static irqreturn_t nv_nic_irq(int foo, void *data)
{
	...
	if (unlikely(test_bit(NV_DO_SPECIAL_DEV_HANDLING, &np->special_dev_handling))) {
		...
		// Its better to take the lock now
		// and check again if the special device handling
		// is done.
		spin_lock(&np->lock);
	
		if (unlikely(test_bit(NV_DO_SPECIAL_DEV_HANDLING, &np->special_dev_handling))) {
			mask = readl(base + NvRegIrqMask);
			printk ("%s: Special dev handling set, mask 0x%x for %s \n", __FUNCTION__, mask, dev->name);
...
#if defined (__VMKLNX__)
		if (unlikely(events & NVREG_IRQ_FATAL_ERROR)) {
			spin_lock(&np->lock);
			/* disable interrupts on the nic */
			writel(0, base + NvRegIrqMask);
			pci_push(base);
...
static void nv_do_special_dev_handling(struct fe_priv *np)
{
	...
	u8 __iomem *base = get_hwbase(dev);
	int i, oom, ret;
	u32 pci_array[0x78/sizeof(u32)];
	u32 saved_config_space[NV_PCI_REGSZ_VER3/4];
	u32 low;
	u32 mask = 0;

	printk(KERN_INFO "%s: Do special device handling for-%s\n", __FUNCTION__, dev->name);
	if (!(spin_trylock(&np->special_dev_handling_lock))) {
		printk(KERN_INFO "%s: Driver busy, re-sched special handling for-%s\n", __FUNCTION__, dev->name);
		schedule_work(&np->special_dev_handling_task);
	}
	...
	*/
	...
	if (np->in_shutdown) {
		clear_bit (NV_DO_SPECIAL_DEV_HANDLING, &np->special_dev_handling);
		spin_unlock(&np->lock);
		netif_tx_unlock_bh(dev);
		return;
	}
	...
	for (i = 0;i <= np->register_size/sizeof(u32); i++)
		saved_config_space[i] = readl(base + i*sizeof(u32));
	...
	for (i = 0;i < 0x78/sizeof(u32); i++)
		pci_write_config_dword(np->pci_dev, i*sizeof(u32), pci_array[i]);
	
	/* restore non-pci configuration space */
	for (i = 0;i <= np->register_size/sizeof(u32); i++)
		writel(saved_config_space[i], base+i*sizeof(u32));
	...
	spin_unlock(&np->special_dev_handling_lock);
	printk(KERN_INFO "%s: Completed special device handling for %s\n", __FUNCTION__, dev->name);
}
#endif /* defined (__VMKLNX__) */
...
static void nv_do_nic_poll(unsigned long data)
{
	...
	if (!(spin_trylock(&np->special_dev_handling_lock))) {
		printk(KERN_INFO "%s: Driver busy special dev handling for %s\n", __FUNCTION__, dev->name);
		return;
	}
	/*
	 * First disable irq(s) and then
...
#if defined (__VMKLNX__)
	if (netif_running(dev) &&  !(np->driver_data & DEV_HAS_POWER_CNTRL)) {
		set_bit(NV_DO_SPECIAL_DEV_HANDLING, &np->special_dev_handling);
		nv_do_special_dev_handling(np);
	}
#endif /* defined (__VMKLNX__) */
...
static void nv_special_dev_handling_task(struct work_struct *work)
{
	struct fe_priv *np = container_of(work,
					struct fe_priv,
					special_dev_handling_task);
	nv_do_special_dev_handling(np);
}
...
static int __devinit nv_probe(struct pci_dev *pci_dev, const struct pci_device_id *id)
{
...
#if defined (__VMKLNX__)
	spin_lock_init(&np->special_dev_handling_lock);
#endif /* defined (__VMKLNX__) */
	...
	INIT_WORK(&np->special_dev_handling_task, nv_special_dev_handling_task);
...
#else
        // Disable TX PAUSE for chip due to bug 258204.
