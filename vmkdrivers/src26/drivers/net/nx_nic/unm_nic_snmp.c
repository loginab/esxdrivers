...
static void read_mau_stats(struct unm_adapter_s *adapter, struct mau_st *mau_stats){
	...
	if(adapter->ahw.board_type == UNM_NIC_XGBE){
		adapter->unm_nic_hw_read_wx(adapter, UNM_NIU_XGE_CONFIG_0 +
			(0x10000*(adapter->physical_port)), &mac_cfg, 4);
...
static void read_dot3stats(struct unm_adapter_s *adapter, struct dot3stats_st * dot3stats){
	...
	/* Transmit and receive fails due to mac sublayer error.
	 */
	dot3stats->mac_transmit_erros = adapter->stats.nocmddescriptor;
	dot3stats->mac_receive_errors = adapter->stats.rcvdbadskb;
}
static void read_pause_stats(struct unm_adapter_s *adapter, struct pause_table_st * pause_stats){
	...
	/* Configuring Pause state not supported, So its value will be equal
	 * to operational mode of pause
	 */
...
unm_nic_snmp_ether_read_proc(char *buf, char **start, off_t offset, int count,
	...
	if(netdev == NULL || ((struct unm_adapter_s*)netdev->priv)->state != PORT_UP) {
