...
int nx_nic_send_cmd_descs(struct net_device *dev, cmdDescType0_t *cmd_desc_arr,
			  int nr_elements);
...
int unm_init_lro(struct unm_adapter_s *adapter) 
{
	...
	rv = nx_mem_pool_create(&adapter->lro.mem_pool, NX_DEFAULT_LRO_ENTRIES,
			   sizeof(lro_entry_t));
	if (rv) {
		nx_nic_print3(NULL, "LRO memory pool creation failed\n");
		return (rv);
	}
	rv = nx_hash_tbl_create(&adapter->lro.hash_tbl, NX_DEFAULT_LRO_ENTRIES,
			   &lro_hash_ops);
	if (rv) {
...
static int nx_lro_send_cleanup(struct unm_adapter_s *adapter)
{
	nx_nic_lro_request_t	*lro_req;
	nic_request_t		req;
	int			rv;
