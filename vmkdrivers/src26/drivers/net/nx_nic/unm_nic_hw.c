 ...
 *
 * $Id: //depot/vmkdrivers/esx40/src26/drivers/net/nx_nic/unm_nic_hw.c#5 $
 *
 */
...
uint32_t unm_nic_get_crbwindow(unm_adapter *adapter)
{
 ...
 */
int
unm_nic_hw_write_w0_128M(unm_adapter *adapter, u64 off, void *data, int len)
{//This is modified from _unm_nic_hw_write()._unm_nic_hw_write does not exist now.
        void *addr;
        unsigned long        flags = 0;
 ...
 */
int
unm_nic_hw_write_w1_128M(unm_adapter *adapter, u64 off, void *data, int len)
{//This is modified from _unm_nic_hw_write()._unm_nic_hw_write does not exist now.
        void *addr;
        unsigned long        flags = 0;
...
unm_nic_hw_write_wx_2M(unm_adapter *adapter, u64 off, void *data, int len)
{
    ...
    if (rv == -1) {
		printk("%s: invalid offset: 0x%016llx\n", __FUNCTION__, off);
		BUG();
		return -1;
	}
...
int
unm_nic_hw_read_w0_128M(unm_adapter *adapter, u64 off, void *data, int len)
{
        void *addr;
        unsigned long        flags = 0;
...
int
unm_nic_hw_read_w1_128M(unm_adapter *adapter, u64 off, void *data, int len)
{
        void *addr;
        unsigned long        flags = 0;
...
void
unm_nic_mem_block_read(struct unm_adapter_s *adapter, u64 off,
                        void *data, int num_words)
{
        ...
        write_lock_irqsave(&adapter->adapter_lock, flags);
	/*
	 * TODO: Check address range validity and alignment.
	 */
        off  = unm_nic_pci_set_window(adapter->ahw.pci_base, off);
        ...
        UNM_NIC_HW_BLOCK_READ_64(data, addr, num_words);
        write_unlock_irqrestore(&adapter->adapter_lock, flags);
...
void
unm_nic_mem_block_write(struct unm_adapter_s *adapter, u64 off,
                            void *data, int num_words)
{
        void *addr;
        unsigned long    flags;
	 ...
	 */
        ...
        UNM_NIC_HW_BLOCK_WRITE_64(data, addr, num_words);
        write_unlock_irqrestore(&adapter->adapter_lock, flags);
...
int
unm_nic_pci_mem_write(struct unm_adapter_s *adapter, u64 off,
                        void *data, int size)
{
        unsigned long   flags;
        void           *addr;
	...
	/*
	 * If attempting to access unknown address range, do not access.
	 */
        if ((off = unm_nic_pci_set_window(adapter, off)) == -1UL){
                size = -1;
        }
        ...
        if(!addr) {
                write_unlock_irqrestore(&adapter->adapter_lock, flags);
                printk(KERN_ERR"%s Tried to access memory outside "
                        "the mapped pci address range: offset is 0x%llx\n", unm_nic_driver_name, off);
                return 1;
        }
...
static int
unm_nic_pci_mem_read(struct unm_adapter_s *adapter,
...
/*static void
 *unm_load_cam_ram(struct unm_adapter_s *adapter)
 *{
 *       DPRINTK(1, INFO, "writing to  LOAD CAM RAM.....START \n");
 *       unm_nic_reg_write(adapter, UNM_CRB_CAM + 0x2000, 0);
...
unm_nic_get_board_info(struct unm_adapter_s *adapter)
{
	...
	if (boardinfo->board_type == UNM_BRDTYPE_P3_4_GB_MM) { 
		gpioval = UNM_CRB_READ_VAL_ADAPTER(UNM_ROMUSB_GLB_PAD_GPIO_I, 
       		                             adapter);
		if ((gpioval & 0x8000) == 0) {
			boardinfo->board_type = UNM_BRDTYPE_P3_10G_TROOPER;
		}
	}
...
unm_nic_macaddr_set(struct unm_adapter_s *adapter, __uint8_t *addr)
{
	...
	if (NX_IS_REVISION_P3(adapter->ahw.revision_id))
        ...
        case UNM_NIC_GBE:
               if ( (((unm_brdtype_t)boardinfo->board_type) == UNM_BRDTYPE_P3_4_GB) ||
                    (((unm_brdtype_t)boardinfo->board_type) == UNM_BRDTYPE_P3_4_GB_MM) ||
                    (((unm_brdtype_t)boardinfo->board_type) == UNM_BRDTYPE_P3_10G_TROOPER)) {
                        //ret = d}nm_niu_gbe_macaddr_set(adapter, addr);
                } else {
                        /*
                         * Flaky Mac address registers on quadgig requires several
                         * writes
                         */
                        for(i = 0; i < retry_count; ++i) {
                                if(unm_niu_gbe_macaddr_set(adapter, addr) != 0)
                                        return -1;

                                unm_niu_gbe_macaddr_get(adapter, (unsigned char *) mac_addr);
                                if(memcmp(mac_addr, addr, 6) == 0)
                                        return 0;
                        }

                        printk(KERN_ERR "%s: Flaky MAC address registers on %s\n",
                            unm_nic_driver_name, adapter->netdev->name);
                }
                break;
        ...
        default:
                dump_stack();
                printk(KERN_ERR"%s: Unknown board type encountered while "
...
#if 0
/* returns FALSE(0) if not acceptable speed else (TRUE)1 */
static int validate_clk_speed(struct unm_adapter_s *adapter)
{
        unm_crbword_t time1=0, time2=0;
        unsigned long jiffies1, jiffies2, jiffies_elapsed, time;

        /* Window 1 read */
        read_lock(&adapter->adapter_lock);
        time1 = UNM_NIC_PCI_READ_32(CRB_NORMALIZE(adapter,UNM_TIMER_TIMESTAMP));
        jiffies1 = jiffies;
        mdelay(1000);
        time2 = UNM_NIC_PCI_READ_32(CRB_NORMALIZE(adapter,UNM_TIMER_TIMESTAMP));
        jiffies2 = jiffies;
        read_unlock(&adapter->adapter_lock);
        ...
        if(jiffies2 > jiffies1)
            jiffies_elapsed = jiffies2 - jiffies1;
        else
            jiffies_elapsed = ~jiffies1 + jiffies2 + 1;

        /* For P2 expected ticks per second is 2048 */
        time = (time * HZ)/jiffies_elapsed;
        return CHECK_TICKS_PER_SEC(time);
}

/* Update CRB register for reflecting correct core clock speed */
int update_core_clock(struct unm_port *port)
{
        struct unm_adapter_s *adapter = port->adapter;
        int                   phy = port->portnum;
        unsigned long jiffies1, jiffies2, jiffies_elapsed, time;
        unm_crbword_t time1=0, time2=0;
        unm_timer_tickctl_t tickctl;

        if ((phy < 0) || (phy > 3))
                return -1;

        if (validate_clk_speed(adapter)) {
            /* No need to worry */
            return 0;
        }
        ...
        /* Window 1 read/write */
        read_lock(&adapter->adapter_lock);
        adapter->unm_nic_hw_read_wx(adapter, UNM_TIMER_GLOBAL_TIMESTAMP_LO, &time1, 4);
        read_unlock(&adapter->adapter_lock);
        jiffies1 = jiffies;
        mdelay(1000);
        read_lock(&adapter->adapter_lock);
        adapter->unm_nic_hw_read_wx(adapter, UNM_TIMER_GLOBAL_TIMESTAMP_LO, &time2, 4);
        read_unlock(&adapter->adapter_lock);
        jiffies2 = jiffies;
        if (time2 > time1)
            time = time2 - time1;
        ...
        /* add 500K, so that sounding of in MHz is correct */
        time = time + 500000UL;
        time /= 1000000UL;
        time = (time * HZ)/jiffies_elapsed;
        if(!ACCEPTABLE_CORE_CLK_RANGE(time)) {
            printk(" Core clock   : %ld MHz, not Acceptable\n", time);
            return -1;
        }
        DPRINTK(1, INFO," Core clock   : %ld MHz\n", time);
        *(unm_crbword_t *)&tickctl = 0;
        tickctl.enable = 1;
        tickctl.count = ((1000000ULL/(4*2048))* time);
        read_lock(&adapter->adapter_lock);
        adapter->unm_nic_hw_read_wx(adapter, UNM_TIMER_GT_TICKCTL, &tickctl, 4);
        read_unlock(&adapter->adapter_lock);
...
unm_nic_set_mtu(struct unm_adapter_s *adapter, int new_mtu)
{
	...
	if (adapter->ahw.revision_id >= NX_P3_A2) {
        ...
        case UNM_NIC_XGBE:
                adapter->unm_nic_hw_read_wx(adapter, UNM_PORT_MODE_ADDR, &port_mode, 4);
                if (port_mode == UNM_PORT_MODE_802_3_AP) {
                    unm_nic_write_w0(adapter, UNM_NIU_AP_MAX_FRAME_SIZE(port), new_mtu);
                } else {
                    if (adapter->physical_port == 0) {
	    	    unm_nic_write_w0 (adapter, UNM_NIU_XGE_MAX_FRAME_SIZE, new_mtu);
                    } else {
	    	    unm_nic_write_w0 (adapter, UNM_NIU_XG1_MAX_FRAME_SIZE, new_mtu);
                    }
                }
                break;
...
unm_nic_enable_mcast_filter(struct unm_adapter_s *adapter)
{
	...
	adapter->unm_crb_writelit_adapter(adapter, UNM_MAC_ADDR_CNTL_REG, val);

	val = 0xffffff;
	adapter->unm_crb_writelit_adapter(adapter, UNM_UNICAST_ADDR(port,0), val);
	adapter->unm_crb_writelit_adapter(adapter, UNM_UNICAST_ADDR(port,0)+4, val);

	memcpy(&val, adapter->mac_addr, 3);
	adapter->unm_crb_writelit_adapter(adapter, UNM_UNICAST_ADDR(port,1), val);

	memcpy(&val, adapter->mac_addr+3, 3);
	adapter->unm_crb_writelit_adapter(adapter, UNM_UNICAST_ADDR(port,1)+4, val);
...
unm_nic_set_mcast_addr(struct unm_adapter_s *adapter, int index, __u8 *addr)
{
	...
	adapter->unm_crb_writelit_adapter(adapter, UNM_MCAST_ADDR(port,index), hi);
	adapter->unm_crb_writelit_adapter(adapter, UNM_MCAST_ADDR(port,index)+4, lo);
                    ...
                    __uint32_t *readval)
{
	if (NX_IS_REVISION_P3(adapter->ahw.revision_id))
        ...
        switch (adapter->ahw.board_type) {
        case UNM_NIC_GBE:
                return unm_niu_gbe_phy_read (adapter, reg, readval);
...
long
unm_nic_phy_write (unm_adapter *adapter, long reg, __uint32_t val)
{
	if (NX_IS_REVISION_P3(adapter->ahw.revision_id))
		return -1;    
        ...
        switch (adapter->ahw.board_type) {
        case UNM_NIC_GBE:
                return unm_niu_gbe_phy_write (adapter, reg, val);
	...
	if (NX_IS_REVISION_P3(adapter->ahw.revision_id)) {
        ...
        case UNM_NIC_XGBE:
                ...
                if (port_mode == UNM_PORT_MODE_802_3_AP) {
                    ...
                    UNM_CRB_READ_CHECK_ADAPTER(UNM_NIU_XGE_CONFIG_1+(0x10000*portnum),
				    		&reg, adapter);
		if (adapter->ahw.revision_id < NX_P3_A2) {
			reg = (reg & ~0x2000UL);
		}
...
unm_nic_stop_port(struct unm_adapter_s *adapter)
{
	if (NX_IS_REVISION_P3(adapter->ahw.revision_id))
        ...
        switch (adapter->ahw.board_type) {
        case UNM_NIC_GBE:
                unm_niu_disable_gbe_port(adapter);
                break;
...
int
unm_crb_write_adapter (unsigned long off, void *data, struct unm_adapter_s *adapter)
{
	return adapter->unm_nic_hw_write_wx(adapter, off, data, 4);
}

int
unm_crb_read_adapter (unsigned long off, void *data, struct unm_adapter_s *adapter)
{
	...
	if (NX_IS_REVISION_P3(adapter->ahw.revision_id)) {
		...
		if ((rv == 0) && status.link) {
			if (nx_fw_cmd_query_phy(adapter, adapter->portnum,
						...
						(unm_crbword_t *)&autoneg)!= 0) {
				adapter->link_autoneg = autoneg;
			}
		}
	} else {
		rv = unm_nic_phy_read(adapter,
				UNM_NIU_GB_MII_MGMT_ADDR_PHY_STATUS,
				(unm_crbword_t *)&status);
		if ((rv == 0) && status.link) {
...
void unm_nic_flash_print(struct unm_adapter_s* adapter)
{
	...
	if(valid) {
        ...
        printk(KERN_INFO "%s %s Board S/N %s  Chip id 0x%x\n",
                        DRIVER_NAME, brd_name, user_info.serial_num,
                        board_info->chip_id);
