...
#define DRIVER_NAME "HP CISS Driver (v 3.6.14.8VMW)"
#define DRIVER_VERSION CCISS_DRIVER_VERSION(3,6,14)

/* Embedded module documentation macros - see modules.h */
MODULE_AUTHOR("Hewlett-Packard Company");
MODULE_DESCRIPTION("Driver for HP Controller SA5xxx SA6xxx version 3.6.14.8VMW");
MODULE_SUPPORTED_DEVICE("HP SA5i SA5i+ SA532 SA5300 SA5312 SA641 SA642 SA6400"
			" SA6i P600 P400 E200 E200i P800 E500");
MODULE_VERSION("3.6.14.8VMW");
MODULE_LICENSE("GPL");
...
static long cciss_char_ioctl(struct inode *inode, struct file *filep,
		       unsigned int cmd, unsigned long arg);
#ifdef CONFIG_COMPAT
static long cciss_char_compat_ioctl(struct file *f, unsigned cmd,
                                    unsigned long arg);
#endif
...
static struct _devmajors devmajors[MAX_CTLR][NWD]; /* hold the major numbers of the char 
 ...
 */
static int
cciss_char_open (struct inode *inode, struct file *filep)
{
	/*
 ...
 */
static const struct file_operations cciss_char_fops = {
	.owner		= THIS_MODULE,
	.open		= cciss_char_open,
	.ioctl		= cciss_char_ioctl,
#ifdef CONFIG_COMPAT
	.compat_ioctl	= cciss_char_compat_ioctl,
#endif
};
...
#if defined (__VMKLNX__)
static long cciss_char_ioctl(struct inode *inode, struct file *f,
                                    unsigned cmd, unsigned long arg)
{
#ifdef CCISS_DEBUG
	printk(KERN_DEBUG "Called with cmd=%x %lx\n", cmd, arg);
#endif
        ...
        /*
         * for the char device, get the corresponding block device
         */
        ...
        /*
         * inode->i_bdev and f->f_dentry are used by the driver in the ioctl
         * path.
         */
        ...
        /*
         * make the actual ioctl call 
         */

        return cciss_ioctl(inode, f, cmd, arg);
                                    ...
                                    unsigned long arg)
{
	...
	printk(KERN_DEBUG "Called with cmd=%x %lx\n", cmd, arg);
        ...
        return cciss_compat_ioctl(f, cmd, arg);
...
#if defined(__VMKLNX__)
		disk->maxXfer = 512 * VMK_SECTOR_SIZE;
		vmklnx_block_register_disk_maxXfer(h->major, drv_index, disk->maxXfer);
#else
					   ...
					   struct pci_dev *pdev, __u32 board_id)
{
	...
	if (pci_find_capability(pdev, PCI_CAP_ID_MSIX)) {
                printk(KERN_WARNING "cciss: MSIX\n");
 ...
 */
	 ...
	 * When configured in ZMR mode, these controllers 
	 * reduce the number of supported commands to 64.
	 * Instead of setting a hardcoded value, we read 
	 * the config table to discover how many commands the 
	 * controller is supporting, then reduce that by 4 to 
	 * allow for ioctl calls.
	 */
...
#if defined(__VMKLNX__)
static void unregister_chrdevs(int ctlr)
{
   int j;
   char* charname = (char*) kmalloc (sizeof(char) * 15, GFP_KERNEL);

   for (j = 0; j < NWD; j++) {
      if (devmajors[ctlr][j].char_major) {
         sprintf(charname, "cciss-c%dd%d", ctlr, j);
         unregister_chrdev(devmajors[ctlr][j].char_major, charname);
      }
   }
   kfree(charname);
}
...
#if defined(__VMKLNX__) 
      /*
       * TODO: kalyanc; try to avoid this
       */
      rc = 0; // Never used
      // we really need a way for return code to return dynamic major number here.
      if (vmklnx_register_blkdev(hba[i]->major, "cciss",
	...
	for (j = 0; j < NWD; j++) {	/* mfm */
		...
		if (!drv->heads && j)
			continue;
                 ...
                 */
                char* charname = (char*) kmalloc (sizeof(char) * 15, GFP_KERNEL); 
                sprintf(charname, "cciss-c%dd%d", i, j);
                devmajors[i][j].char_major = register_chrdev(0, charname, &cciss_char_fops);
                if (devmajors[i][j].char_major < 0) {
                        printk(KERN_WARNING
                                        "cciss: failed to register char device %s\n",
                                        charname);
                }
                kfree(charname);
                devmajors[i][j].block_major = hba[i]->major;
                devmajors[i][j].block_minor = disk->first_minor;
                devmajors[i][j].bd_disk = disk;
#endif
		...
		add_disk(disk);
#if defined(__VMKLNX__)
		disk->maxXfer = 4096*31;
#endif /*  defined(__VMKLNX__)  */
	}
#if defined(__VMKLNX__)
	vmklnx_block_register_sglimit(hba[i]->major, MAXSGENTRIES);
	vmklnx_block_init_done(hba[i]->major);
#endif /*  defined(__VMKLNX__)  */
