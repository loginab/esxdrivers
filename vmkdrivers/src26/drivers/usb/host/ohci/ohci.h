...
/* These #defines are moved outside the "struct td" to avoid the random 
 * compilation error related to stray "#". The error was seen after we
 * added the VMK_PADDED_STRUCT macro in the struct. It is randomly seen
 * and is probably related to the compilers order of parsing.
 */ 
...
#ifdef __VMK_GCC_BUG_ALIGNMENT_PADDING__
VMK_ASSERT_LIST(OHCI_STRUCT_ALIGN,
   VMK_ASSERT_ON_COMPILE(sizeof(struct td)%32 == 0);
)
#endif
