...
// XXX The above _must_ be a power of 2 for the below to work (consider the &)
#undef THREADSAFE_INCR_AND_MODULUS
#define THREADSAFE_INCR_AND_MODULUS(a, b) (a) = ((a)+1) & ((b)-1)

struct usb_stor_msg_buf usb_stor_default_msgbuf;

void usb_stor_add_message_to_buffer(struct usb_stor_msg_buf *msgbuf,
	char * format, ...)
{
	va_list args;

	// Ignore snprintf failures here and catch them when we print, if ever
	va_start(args, format);
	vsnprintf(msgbuf->buffer[msgbuf->tail],
		USB_STOR_MSG_LEN,
		format,
		args);
	va_end(args);

	THREADSAFE_INCR_AND_MODULUS(msgbuf->tail,USB_STOR_BUFF_SIZE);
	if (msgbuf->tail == msgbuf->head) {
		THREADSAFE_INCR_AND_MODULUS(msgbuf->head, USB_STOR_BUFF_SIZE);
	}
}

void usb_stor_walk_and_print_warning_buffer(struct usb_stor_msg_buf *msgbuf)
{
	while (msgbuf->head != msgbuf->tail) {
		// XXX Overkill, but guard against races & snprintf failures
		msgbuf->buffer[msgbuf->head][USB_STOR_MSG_LEN] = 0;

		printk("                         %s",
			msgbuf->buffer[msgbuf->head]);
		THREADSAFE_INCR_AND_MODULUS(msgbuf->head, USB_STOR_BUFF_SIZE);
	}
}
#endif
...
#if defined(__VMKLNX__)
{
	usb_stor_show_command_data(srb, usb_stor_show_command_name(srb));
}
...
#if defined(__VMKLNX__)
	default: if (snprintf(buf, BUF_SIZE, USB_STOR_UNKNOWN_COMMAND ": 0x%x)",
				 srb->cmnd[0]))
		 	what = buf;
		 else
			what = USB_STOR_UNKNOWN_COMMAND ")";
		 break;
#else
