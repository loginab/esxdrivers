...
#include "vmklinux26_usb.h"

extern long wait_for_urb_completion(struct urb *, struct completion *);
#endif
...
static int usb_stor_msg_common(struct us_data *us, int timeout)
{
	...
	if (us->current_urb->transfer_buffer == us->sensebuf) {
		us->current_urb->transfer_flags |= URB_NO_TRANSFER_DMA_MAP;
		us->current_urb->transfer_dma = us->sensebuf_dma;
	}
#else
	if (us->current_urb->transfer_buffer == us->iobuf)
		us->current_urb->transfer_flags |= URB_NO_TRANSFER_DMA_MAP;
...
#if defined(__VMKLNX__)
	if (us->srb && (unlikely(scsi_dump_active(us->srb)))) {
		timeleft = wait_for_urb_completion(
			us->current_urb, &urb_done);
	} else /* no curly brace here so only the next stmt is in the else */
#endif
		...
		unsigned int length, int result, unsigned int partial)
{
	...
	case -EPIPE:
		...
		_VMKLNX_USB_STOR_WARN("clearing endpoint halt for pipe 0x%x\n", us->srb, pipe);
	...
	case -EOVERFLOW:
		_VMKLNX_USB_STOR_WARN("-- babble\n", us->srb);
		US_DEBUGP("-- babble\n");
		return USB_STOR_XFER_LONG;

	/* the transfer was cancelled by abort, disconnect, or timeout */
	case -ECONNRESET:
		_VMKLNX_USB_STOR_WARN("-- transfer cancelled\n", us->srb);
		US_DEBUGP("-- transfer cancelled\n");
		return USB_STOR_XFER_ERROR;

	/* short scatter-gather read transfer */
	case -EREMOTEIO:
		//XXX Probably can demote this when usb stack is stable
		_VMKLNX_USB_STOR_WARN("-- short read transfer\n", us->srb);
	...
	case -EIO:
		//XXX Probably can demote this when usb stack is stable
		_VMKLNX_USB_STOR_WARN("-- abort or disconnect in progress\n", us->srb);
		US_DEBUGP("-- abort or disconnect in progress\n");
		return USB_STOR_XFER_ERROR;

	/* the catch-all error case */
	default:
		_VMKLNX_USB_STOR_WARN("-- unknown error\n", us->srb);
...
/**
 * usb_stor_restore_cmnd - Restore a scsi command info as part of error recovery
 * @scmd:       SCSI command structure to restore
 * @ses:        saved information from a coresponding call to usb_stor_prep_cmd
 *
 * Undo any damage done by above usb_stor_prep_cmd().
 **/
static void usb_stor_restore_cmnd(struct scsi_cmnd* scmd, struct scsi_eh_save *ses)
{
	/*
	...
	memcpy(scmd->sense_buffer, scmd->request_buffer, scmd->request_bufflen);
...
{
	_VMKLNX_USB_STOR_WARN("%s called\n", us->srb, __FUNCTION__);
	US_DEBUGP("%s called\n", __FUNCTION__);
...
#if defined(__VMKLNX__)
		do {
			int status = usb_unlink_urb(us->current_urb);
			if (status && status != -EINPROGRESS) {
				_VMKLNX_USB_STOR_WARN("-- abort raced, unlink_urb returned %d for urb with status %d unlink\n", us->srb, status, us->current_urb->status);
			} else if (us->srb->result != DID_ABORT << 16) {
				_VMKLNX_USB_STOR_WARN("-- unexpected us->srb->result %x for successfully aborted srb %p\n", us->srb, us->srb->result, us->srb);
			}
		} while (0);
#else
		usb_unlink_urb(us->current_urb);
	...
	if (test_and_clear_bit(US_FLIDX_SG_ACTIVE, &us->flags)) {
		_VMKLNX_USB_STOR_WARN("-- cancelling sg request\n", us->srb);
		US_DEBUGP("-- cancelling sg request\n");
		usb_sg_cancel(&us->current_sg);
	}
}
