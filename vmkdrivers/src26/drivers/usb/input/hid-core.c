...
static int hid_submit_out(struct hid_device *hid)
{
	...
	hid_output_report(report, (__u8 *)usbhid->outbuf);
...
static int hid_submit_ctrl(struct hid_device *hid)
{
	...
	if (dir == USB_DIR_OUT) {
		hid_output_report(report, (__u8 *)usbhid->ctrlbuf);
...
static struct hid_device *usb_hid_configure(struct usb_interface *intf)
{
	...
	if (!(hid = hid_parse_report((__u8 *)rdesc, n))) {
	...
	hid_find_max_report(hid, HID_INPUT_REPORT, (int *)&usbhid->bufsize);
	hid_find_max_report(hid, HID_OUTPUT_REPORT, (int *)&usbhid->bufsize);
	hid_find_max_report(hid, HID_FEATURE_REPORT, (int *)&usbhid->bufsize);
