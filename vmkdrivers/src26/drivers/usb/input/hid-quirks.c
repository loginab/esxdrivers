...
#if defined(__VMKLNX__)
static vmk_Semaphore dquirks_rwsem;
#else
...
#if defined(__VMKLNX__)
	VMK_ReturnStatus status = vmk_SemaCreate(&dquirks_rwsem, vmk_ModuleStackTop(), "dquirks_rwsem", 1);
	if (status != VMK_OK)
		return -EINVAL;
#endif
...
static void __usbhid_fixup_report_descriptor(__u32 quirks, char *rdesc, unsigned rsize)
{
	...
	if (quirks & HID_QUIRK_RDESC_LOGITECH)
		usbhid_fixup_logitech_descriptor((unsigned char *)rdesc, rsize);

	if (quirks & HID_QUIRK_RDESC_SWAPPED_MIN_MAX)
		usbhid_fixup_cypress_descriptor((unsigned char *)rdesc, rsize);

	if (quirks & HID_QUIRK_RDESC_PETALYNX)
		usbhid_fixup_petalynx_descriptor((unsigned char *)rdesc, rsize);

	if (quirks & HID_QUIRK_RDESC_MACBOOK_JIS)
		usbhid_fixup_macbook_descriptor((unsigned char *)rdesc, rsize);
}
