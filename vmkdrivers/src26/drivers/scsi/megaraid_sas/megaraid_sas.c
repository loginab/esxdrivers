		   ...
		   struct megasas_cmd *cmd)
{
	...
	if(scp->device->type == TYPE_TAPE) {
		pthru->timeout = scp->timeout_per_command;
	}
...
#if defined(__VMKLNX__) // VMkernel maps all channels to channel 0
	    (MEGASAS_TARGET_TO_DEV(scmd->device->id) >= MEGASAS_MAX_LD ||
	     scmd->device->lun)) {
...
static int megasas_wait_for_outstanding(struct megasas_instance *instance)
{
	...
	if (instance->is_busy) { // already in waiting
...
static int megasas_generic_reset(struct scsi_cmnd *scmd)
{
	...
	scmd_printk(KERN_NOTICE, scmd, "megasas: RESET sn %ld cmd=%x retries=%x\n",
	...
	if (ret_val == SUCCESS)
		...
		printk(KERN_ERR "megasas: failed to do reset, mark controller DEAD\n");
...
static void megasas_hotplug_work(void *arg)
{
	struct megasas_hotplug_event *ev = arg;
...
#define MR_EVT_LD_OFFLINE		0x00fc
	...
	/*
	 * Register AEN with FW for the next event
	 */
...
megasas_service_aen(struct megasas_instance *instance, struct megasas_cmd *cmd)
{
	...
	if (!ev) {
		...
		INIT_WORK(&ev->hotplug_work, megasas_hotplug_work, ev);
...
#if !defined(__VMKLNX__) // We don't support this field in vmklinux26 today
...
#if defined(__VMKLNX__) 
	/*
	 * PR200226: vmkernel only supports channel 0 today so we map all
	 * channels to channel 0 for now.
	 */
        host->max_channel = 0;
