 ...
 */
#define MEGASAS_VERSION				"00.00.03.21.11vmw"
...
/*
 * PR215909: The adapter fw is reported physical LUNs to the driver,
 * causing vmklinux or storage stack mistakenly adding them to vmkernel.
 * We should only see the logical LUN. Hence block physical disk channels.
 */
#define MEGASAS_MAX_PD_CHANNELS			0
...
#if defined(__VMKLNX__)
/*
 * VMkernel scsi drivers can only have one channel.  So here we map the
 * two physical and two logical channels to channel 0, with the logical
 * channels coming first.   In other words:
 *
 *	ORIGINAL DRIVER			VMKERNEL DRIVER
 *	------------------------	-------------------------
 *	Channel 0, targets 0-127	Channel 0 targets 256-383
 *	Channel 1, targets 0-127	Channel 0 targets 384-511
 *	Channel 2, targets 0-127	Channel 0 targets 0-127
 *	Channel 3, targets 0-127	Channel 0 targets 128-255
 *
 */
#define MEGASAS_IS_LOGICAL(scp)						\
	(scp->device->id < (MEGASAS_MAX_LD_CHANNELS * MEGASAS_MAX_DEV_PER_CHANNEL))

#define MEGASAS_DEV_INDEX(inst, scp)					\
	(scp->device->id % (2 * MEGASAS_MAX_DEV_PER_CHANNEL))

#define MEGASAS_TARGET_TO_DEV(tgt)					\
	((tgt) % MEGASAS_MAX_DEV_PER_CHANNEL)
