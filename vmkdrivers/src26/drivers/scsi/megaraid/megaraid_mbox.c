	...
	if (islogical) {
		...
		case MODE_SENSE:
			if (scp->use_sg) {
				...
				for (i = 0; 
				     i < scp->use_sg && len > 0; i++, sgl++) {
					temp = phys_to_virt(sg_dma_address(sgl));
					memset(temp, 0, sg_dma_len(sgl));
					len -= sg_dma_len(sgl);
				}
...
#if defined(__VMKLNX__)
		/*
		 * VMFS requires RESERVE/RELEASE support, but MegaRAID doesn't
		 * support them, so fake them.
		 */
		case RESERVE_10:
		case RELEASE_10:
