...
#define MAX_DEVS		(HPSA_MAX_LUN + MAX_TARGETS)
...
typedef struct _ReportLUNdata_struct
{
  ...
  DWORD reserved;
  BYTE LUN[HPSA_MAX_LUN][8];
} ReportLunData_struct;

typedef struct _ReportExtendedLUNdata_struct
{
  BYTE LUNListLength[4];
  BYTE extended_response_flag;
  BYTE reserved[3];
  BYTE LUN[HPSA_MAX_LUN][24];
} ReportExtendedLunData_struct;

typedef struct _SenseSubsystem_info_struct
{
        BYTE reserved[36];
        BYTE portname[8];
        BYTE reserved1[1108];
} SenseSubsystem_info_struct;
