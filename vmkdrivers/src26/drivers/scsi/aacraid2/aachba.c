...
int cmd_abort_timeout = 5; /* default value - start with 5 secs */
module_param(cmd_abort_timeout, int, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(cmd_abort_timeout, "The duration (in secs) that the abort"
	" handler waits for commands that are still owned by firmware. \n"
	"Typically it should in 5-10 secs range.");
...
#if defined(__VMKLNX__)
	/*
	 * scsi_sg_copy_from_buffer() used here to simplify the code and
	 * also: 
	 *  - aac_internal_transfer() purpose is to copy data to scsicmd's
	 *    request_buffer or SG list
	 *  - all callers pass offset of 0 except two of them, so just convert 
	 *    that caller to completely avoid offset.
	 */
	scsi_sg_copy_from_buffer(scsicmd, data, len);
...
#if !defined(__VMKLNX__)
	/*
	 * avoid calling get_fs which is currently unimplemeneted in vmklinux26.
	 */  
#ifndef MODULE
...
#else /* defined(__VMKLNX__) */
	   /*
	    * num_physpages is a global variable that linux uses to track the
	    * number of physical pages on the system. vmklinux26 does not
	    * support such a count - so that part of the if-statement was
	    * removed.
	    */
...
#else /* defined(__VMKLNX__) */
	/*
	 * dma_get_required_mask is not implemented in vmklinux yet, but we can 
	 * just assume that we need 64bit addressing
	 */
	dev->needs_dac = 1;
#endif /* !defined(__VMKLNX__) */
	...
	u32 count)
{
	...
	dprintk((KERN_DEBUG "l=%llu %llu[%u] q=%u %lu\n",
	  dev->fsa_dev[cid].last, lba, count, device->queue_depth,
	  (unsigned long)(dev->queues->queue[AdapNormCmdQueue].numpending)));
#endif
...
#if !defined(__VMKLNX__)
	/* comment this block of code to have same/compatible signature as old driver */
...
#if defined(__VMKLNX__)
			/*
			 * the purpose of the ifdef'd out code is to fill the rest
			 * of the request buffer with 0's, this simple piece code
			 * does same thing without all those offset calculations!
			 */
			char *buf = kzalloc(alloc_len, GFP_KERNEL);
			if (buf) {
				scsi_sg_copy_to_buffer(scsicmd, buf, sizeof(cp));
				scsi_sg_copy_from_buffer(scsicmd, buf, alloc_len);
				kfree(buf);
			} else {
			/*
			 * okay in case kzalloc fails, it's fine. Because I think
			 * when we get large request buffer than needed we shouldn't
			 * be filling the rest of the scmd's buffer with 0's,
			 * instead just setting 'resid' of scmd suffice.
			 */
				scsi_set_resid(scsicmd, alloc_len - sizeof(cp));
			}
#else /* !defined(__VMKLNX__) */
...
#if !defined(__VMKLNX__)
			/*
			 * This code was removed because in vmklinux26,
			 * scsicmd->request->rq_disk is a dangling pointer
			 * and is not always NULL (but is some bogus value).
			 * 
			 * This leads to memory corruption or a PSOD when it is
			 * dereferenced as the target of the strlcpy
			 */
...
#endif
#ifdef INITFLAGS_BEDROC_SUPPORTED
