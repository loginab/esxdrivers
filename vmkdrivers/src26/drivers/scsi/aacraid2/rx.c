...
static int aac_rx_deliver_apre(struct fib * fib)
{
	...
	spin_lock_irqsave(q->lock, qflags);
	if (unlikely((q->numpending >= q->entries) || ((q->Credits - fib->Credits) < 0))) {
		spin_unlock_irqrestore(q->lock, qflags);
		return -EBUSY;
	}
...
int _aac_rx_init(struct aac_dev *dev)
{
	...
	/*
	 * reset_devices is eliminated from this if statement beceuase it is a
	 * global variable used in the linux kernel which is unsupported in
	 * vmklinux26
	 */
