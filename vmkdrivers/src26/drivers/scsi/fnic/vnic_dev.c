 ...
 *
 */
#ident "$Id: vnic_dev.c 21747 2008-12-10 06:54:17Z kanna $"
...
#ifndef NOT_FOR_UPSTREAM_KERNEL
#include "kcompat.h"
#endif
...
#ifdef CONFIG_MIPS
		vdev->res[type].vaddr = pci_iomap(vdev->pdev, bar_num, len);
#else
	...
	void *value)
{
	...
	if (!v) {
		printk(KERN_ERR
			"vNIC device-specific region not found.\n");
		return -EINVAL;
	}

	switch (size) {
	case 1: *(u8 *)value = ioread8(v + offset); break;
	case 2: *(u16 *)value = ioread16(v + offset); break;
	case 4: *(u32 *)value = ioread32(v + offset); break;
	case 8: *(u64 *)value = readq(v + offset); break;
	default: BUG(); break;
	}
