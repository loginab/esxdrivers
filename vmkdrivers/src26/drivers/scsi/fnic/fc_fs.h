 ...
 * SOFTWARE.
 *
 * $Id: fc_fs.h 19896 2008-10-24 21:08:41Z kanna $
 *
 */
...
struct fc_frame_header {
	net8_t          fh_r_ctl;	/* routing control */
	net24_t         fh_d_id;	/* Destination ID */

	net8_t          fh_cs_ctl;	/* class of service control / pri */
	net24_t         fh_s_id;	/* Source ID */

	net8_t          fh_type;	/* see enum fc_fh_type below */
	net24_t         fh_f_ctl;	/* frame control */

	net8_t          fh_seq_id;	/* sequence ID */
	net8_t          fh_df_ctl;	/* data field control */
	net16_t         fh_seq_cnt;	/* sequence count */

	net16_t         fh_ox_id;	/* originator exchange ID */
	net16_t         fh_rx_id;	/* responder exchange ID */
	net32_t         fh_parm_offset;	/* parameter or relative offset */
};
...
static inline int fc_mfs_valid(size_t mfs)
{
	return (mfs >= FC_MIN_MAX_FRAME);
}
 ...
 */
struct fc_ba_acc {
	net8_t		ba_seq_id_val;	/* SEQ_ID validity */
#define FC_BA_SEQ_ID_VAL 0x80
	net8_t		ba_seq_id;	/* SEQ_ID of seq last deliverable */
	net8_t		ba_resvd[2];	/* reserved */
	net16_t		ba_ox_id;	/* OX_ID for aborted seq or exch */
	net16_t		ba_rx_id;	/* RX_ID for aborted seq or exch */
	net16_t		ba_low_seq_cnt;	/* low SEQ_CNT of aborted seq */
	net16_t		ba_high_seq_cnt; /* high SEQ_CNT of aborted seq */
};
 ...
 */
struct fc_ba_rjt {
	net8_t		br_resvd;	/* reserved */
	net8_t		br_reason;	/* reason code */
	net8_t		br_explan;	/* reason explanation */
	net8_t		br_vendor;	/* vendor unique code */
};
...
enum fc_ba_rjt_reason {
	...
	FC_BA_RJT_VENDOR =	0xff,	/* vendor-specific error (see br_vendor) */
};
 ...
 */
struct fc_pf_rjt {
	net8_t		rj_action;	/* reserved */
	net8_t		rj_reason;	/* reason code */
	net8_t		rj_resvd;	/* reserved */
	net8_t		rj_vendor;	/* vendor unique code */
};
 ...
 */
struct fc_data_desc {
	net32_t		dd_offset;	/* data relative offset in bytes */
	net32_t		dd_len;		/* transfer buffer size in bytes */
	net8_t		_dd_resvd[4];
};
