 ...
 * SOFTWARE.
 *
 * $Id: fc_encaps.h 19896 2008-10-24 21:08:41Z kanna $
 */
...
struct fc_encaps_hdr {
    net8_t      fc_proto;               /* protocol number */
    net8_t      fc_ver;                 /* version of encapsulation */
    net8_t      fc_proto_n;             /* ones complement of protocol */
    net8_t      fc_ver_n;               /* ones complement of version */

    u_char      fc_proto_data[8];       /* protocol specific data */

    net16_t     fc_len_flags;           /* 10-bit length/4 w/ 6 flag bits */
    net16_t     fc_len_flags_n;         /* ones complement of length / flags */

    /*
     * Offset 0x10
     */
    net32_t     fc_time[2];             /* time stamp: seconds and fraction */
    net32_t     fc_crc;                 /* CRC */
    net32_t     fc_sof;                 /* start of frame (see FC_SOF below) */
