 ...
 * SOFTWARE.
 *
 * $Id: fc_exch.h 18557 2008-09-14 22:36:38Z jre $
 */
...
/*
 * Set a timer on an exchange and set process-type error policy.
 * The time is a minimum delay in milliseconds until the timer fires.
 */
void fc_exch_timer_set_recover(struct fc_exch *, u_int timer_msec);
 ...
 */
int fc_seq_abort_exch(struct fc_seq *);
...
/*
 * Get exchange IDs for debugging or recovery.
 */
void fc_seq_get_xids(struct fc_seq *, fc_xid_t *, fc_xid_t *);

/*
 * Set FC-4 recovery data for sequence's exchange.
 */
void fc_seq_rec_data(struct fc_seq *, uint32_t);

/*
 * Handle incoming ELS requests.
 */
void fc_exch_mgr_els_rec(struct fc_seq *, struct fc_frame *);
void fc_exch_mgr_els_rrq(struct fc_seq *, struct fc_frame *);

/*
 * Handle incoming BLS frames.
 */
void fc_exch_recv_bls(struct fc_exch_mgr *, struct fc_frame *);
