 ...
 * SOFTWARE.
 *
 * $Id: fc_els.h 20444 2008-11-07 00:36:17Z herman $
 *
 */
...
#define	FC_SP_FT_NNPIV	0x8000	/* N_port can use NPIV (FLOGI req. only) */
#define	FC_SP_FT_RAND	0x4000	/* random relative offset */
#define	FC_SP_FT_VAL	0x2000	/* valid vendor version level */
#define	FC_SP_FT_FNPIV	0x2000	/* F_port handles NPIV (FLOGI LS_ACC only) */
...
struct fc_els_flogi {
	...
	net64_t		fl_wwnn;	/* fabric or node name */
 ...
 */
struct fc_els_lka {
	net8_t          lk_cmd;		/* command code ELS_LS_LKA */
	net8_t          lk_resv[3];	/* reserved */
};
...
struct fc_els_adisc {
	net8_t          adisc_cmd;
	net8_t          adisc_resv[3];  /* reserved */
	net8_t          adisc_resv1;
	net24_t         adisc_hard_addr;
	net64_t         adisc_wwpn;
	net64_t         adisc_wwnn;
	net8_t          adisc_resv2;
	net24_t         adisc_port_id;
};
