 ...
 * SOFTWARE.
 */
#ident "$Id: cq_exch_desc.h 18893 2008-09-25 02:03:16Z gsapozhnikov $"
...
struct cq_exch_wq_desc {
    ...
    u_int16_t q_number;
    u_int16_t exchange_id;
    u_int8_t tmpl;
    u_int8_t reserved0;
    u_int32_t reserved1;
    u_int8_t exch_status;
...
enum cq_exch_status_types {
    ...
    CQ_EXCH_WQ_STATUS_TYPE_SGL_EOF = 2,     /* XXX-HW substitute for now */
    CQ_EXCH_WQ_STATUS_TYPE_TMPL_ERR = 3,    /* XXX-HW substitute for now */
};

static __inline__ void cq_exch_wq_desc_dec(struct cq_exch_wq_desc *desc_ptr,
                                           u_int8_t *type,
                                           u_int8_t *color,
                                           u_int16_t *q_number,
                                           u_int16_t *completed_index,
...
struct cq_fcp_rq_desc {
    u_int16_t completed_index_eop_sop_prt;
    u_int16_t q_number;
    u_int16_t exchange_id;
    u_int16_t tmpl;
    u_int16_t bytes_written;
    u_int16_t vlan;
    u_int8_t sof;
    u_int8_t eof;
    u_int8_t fcs_fer_fck;
    u_int8_t type_color;
};
...
#define CQ_FCP_RQ_DESC_PACKET_ERR_MASK \
                                        (1 << CQ_FCP_RQ_DESC_PACKET_ERR_SHIFT)
#define CQ_FCP_RQ_DESC_VS_STRIPPED_SHIFT            15
#define CQ_FCP_RQ_DESC_VS_STRIPPED_MASK \
                                        (1 << CQ_FCP_RQ_DESC_VS_STRIPPED_SHIFT)
#define CQ_FCP_RQ_DESC_FC_CRC_OK_MASK               0x1
#define CQ_FCP_RQ_DESC_FCOE_ERR_SHIFT               1
#define CQ_FCP_RQ_DESC_FCOE_ERR_MASK \
                                        (1 << CQ_FCP_RQ_DESC_FCOE_ERR_SHIFT)
#define CQ_FCP_RQ_DESC_FCS_OK_SHIFT                 7
#define CQ_FCP_RQ_DESC_FCS_OK_MASK \
                                        (1 << CQ_FCP_RQ_DESC_FCS_OK_SHIFT)

static __inline__ void cq_fcp_rq_desc_dec(struct cq_fcp_rq_desc *desc_ptr,
                                          u_int8_t *type,
                                          u_int8_t *color,
                                          u_int16_t *q_number,
                                          u_int16_t *completed_index,
                                          ...
                                          u_int8_t *sof,
                                          u_int8_t *eof,
                                          u_int8_t *ingress_port,
                                          u_int8_t *packet_err,
                                          u_int8_t *fcoe_err,
                                          u_int8_t *fcs_ok,
                                          u_int8_t *vlan_stripped,
                                          u_int16_t *vlan)
{
    cq_desc_dec((struct cq_desc *)desc_ptr, type,
      color, q_number, completed_index);
    *eop = (desc_ptr->completed_index_eop_sop_prt & CQ_FCP_RQ_DESC_FLAGS_EOP) ?
        1: 0;
	*sop = (desc_ptr->completed_index_eop_sop_prt & CQ_FCP_RQ_DESC_FLAGS_SOP) ?
		1 : 0;
	*ingress_port = 
        (desc_ptr->completed_index_eop_sop_prt & CQ_FCP_RQ_DESC_FLAGS_PRT) ?
		1 : 0;
...
struct cq_sgl_desc {
    u_int16_t exchange_id;
    u_int16_t q_number;
    u_int32_t active_burst_offset;
    u_int32_t tot_data_bytes;
    u_int16_t tmpl;
    u_int8_t sgl_err;
    u_int8_t type_color;
};
...
enum cq_sgl_err_types {
    ...
    CQ_SGL_ERR_DATA_LCL_ADDR_ERR,   /* sgl data buffer to local vnic addr ill */
...
static __inline__ void cq_sgl_desc_dec(struct cq_sgl_desc *desc_ptr,
                                       u_int8_t *type,
                                       u_int8_t *color,
                                       u_int16_t *q_number,
                                       u_int16_t *exchange_id,
                                       ...
                                       u_int8_t *sgl_err)
{
    /* Cheat a little by assuming exchange_id is the same as completed index */
    cq_desc_dec((struct cq_desc *)desc_ptr, type, color, q_number, exchange_id);
