 ...
 * SOFTWARE.
 */
#ident "$Id: fcpio.h 19265 2008-10-08 23:24:53Z ajoglekar $"
...
static inline void
fcpio_tag_id_enc(struct fcpio_tag *tag, u_int32_t id)
{
    tag->u.req_id = id;
}

static inline void
fcpio_tag_id_dec(struct fcpio_tag *tag, u_int32_t *id)
{
    *id = tag->u.req_id;
}

static inline void
fcpio_tag_exid_enc(struct fcpio_tag *tag, u_int16_t ox_id, u_int16_t rx_id)
{
...
static inline void
fcpio_tag_exid_dec(struct fcpio_tag *tag, u_int16_t *ox_id, u_int16_t *rx_id)
{
 ...
 */
struct fcpio_header {
    u_int8_t            type;           /* enum fcpio_type */
    u_int8_t            status;         /* header status entry */
    u_int16_t           _resvd;         /* reserved */
...
struct fcpio_icmnd_16 {
    u_int32_t   lunmap_id;              /* index into lunmap table */
    u_int8_t    special_req_flags;      /* special exchange request flags */
    u_int8_t    _resvd0[3];             /* reserved */
    u_int32_t   sgl_cnt;                /* scatter-gather list count */
    u_int32_t   sense_len;              /* sense buffer length */
    u_int64_t   sgl_addr;               /* scatter-gather list addr */
    u_int64_t   sense_addr;             /* sense buffer address */
    u_int8_t    crn;                    /* SCSI Command Reference No. */
    u_int8_t    pri_ta;                 /* SCSI Priority and Task attribute */
    u_int8_t    _resvd1;                /* reserved: should be 0 */
    u_int8_t    flags;                  /* command flags */
    u_int8_t    scsi_cdb[CDB_16];       /* SCSI Cmnd Descriptor Block */
    u_int32_t   data_len;               /* length of data expected */
    u_int8_t    lun[LUN_ADDRESS];       /* FC vNIC only: LUN address */
    u_int8_t    _resvd2;                /* reserved */
    net24_t     d_id;                   /* FC vNIC only: Target D_ID */
    u_int16_t   mss;                    /* FC vNIC only: max burst */
    u_int16_t   _resvd3;                /* reserved */
    u_int32_t   r_a_tov;                /* FC vNIC only: Res. Alloc Timeout */
    u_int32_t   e_d_tov;                /* FC vNIC only: Error Detect Timeout */
};
...
struct fcpio_icmnd_32 {
    ...
    u_int8_t    _resvd1;                /* reserved: should be 0 */
    u_int8_t    flags;                  /* command flags */
    u_int8_t    scsi_cdb[CDB_32];       /* SCSI Cmnd Descriptor Block */
    u_int32_t   data_len;               /* length of data expected */
    u_int8_t    lun[LUN_ADDRESS];       /* FC vNIC only: LUN address */
    u_int8_t    _resvd2;                /* reserved */
    net24_t     d_id;                   /* FC vNIC only: Target D_ID */
...
struct fcpio_itmf {
    u_int32_t   lunmap_id;              /* index into lunmap table */
    u_int32_t   tm_req;                 /* SCSI Task Management request */
    u_int32_t   t_tag;                  /* header tag of fcpio to be aborted */
    u_int32_t   _resvd;                 /* _reserved */
    u_int8_t    lun[LUN_ADDRESS];       /* FC vNIC only: LUN address */
    u_int8_t    _resvd1;                /* reserved */
    net24_t     d_id;                   /* FC vNIC only: Target D_ID */
    u_int32_t   r_a_tov;                /* FC vNIC only: R_A_TOV in msec */
    u_int32_t   e_d_tov;                /* FC vNIC only: E_D_TOV in msec */
};
 ...
 */
struct fcpio_tdata {
    u_int16_t   rx_id;                  /* FC rx_id of target command */
    u_int16_t   flags;                  /* command flags */
    u_int32_t   rel_offset;             /* data sequence relative offset */
    u_int32_t   sgl_cnt;                /* scatter-gather list count */
    u_int32_t   data_len;               /* length of data expected to send */
    u_int64_t   sgl_addr;               /* scatter-gather list address */
};
...
struct fcpio_trsp {
    u_int16_t   rx_id;                  /* FC rx_id of target command */
    u_int16_t   _resvd0;                /* reserved */
    u_int32_t   sense_len;              /* sense data buffer length */
    u_int64_t   sense_addr;             /* sense data buffer address */
    u_int16_t   _resvd1;                /* reserved */
    u_int8_t    flags;                  /* response request flags */
    u_int8_t    scsi_status;            /* SCSI status */
    u_int32_t   residual;               /* SCSI data residual value of I/O */
};
...
struct fcpio_ttmf_ack {
    u_int16_t   rx_id;                  /* FC rx_id of target command */
    u_int16_t   _resvd0;                /* reserved */
    u_int32_t   tmf_status;             /* SCSI task management status */
};
 ...
 */
struct fcpio_tabort {
    u_int16_t   rx_id;                  /* rx_id of the target request */
};
...
struct fcpio_flogi_reg {
    u_int8_t    format;
    net24_t     s_id;                   /* FC vNIC only: Source S_ID */
    net48_t     gateway_mac;            /* Destination gateway mac */
    u_int16_t   _resvd;
    u_int32_t   r_a_tov;                /* R_A_TOV in msec */
    u_int32_t   e_d_tov;                /* E_D_TOV in msec */
};
...
struct fcpio_lunmap_req {
    u_int64_t addr;                     /* address of the buffer */
    u_int32_t len;                      /* len of the buffer */
};
...
struct fcpio_flogi_fip_reg {
    u_int8_t    _resvd0;
    net24_t     s_id;                   /* FC vNIC only: Source S_ID */
    net48_t     fcf_mac;                /* FCF Target destination mac */
    u_int16_t   _resvd1;
    u_int32_t   r_a_tov;                /* R_A_TOV in msec */
    u_int32_t   e_d_tov;                /* E_D_TOV in msec */
    net48_t     ha_mac;                 /* Host adapter source mac */
    u_int16_t   _resvd2;
};
...
struct fcpio_host_req {
    ...
    union {
         ...
         */
        u_int8_t buf[FCPIO_HOST_REQ_LEN - sizeof (struct fcpio_header)];
...
struct fcpio_icmnd_cmpl {
    u_int8_t    _resvd0[6];             /* reserved */
    u_int8_t    flags;                  /* response flags */
    u_int8_t    scsi_status;            /* SCSI status */
    u_int32_t   residual;               /* SCSI data residual length */
    u_int32_t   sense_len;              /* SCSI sense length */
};
...
struct fcpio_tcmnd_16 {
    ...
    u_int8_t    crn;                    /* SCSI Command Reference No. */
    u_int8_t    pri_ta;                 /* SCSI Priority and Task attribute */
    u_int8_t    _resvd2;                /* reserved: should be 0 */
    u_int8_t    flags;                  /* command flags */
    u_int8_t    scsi_cdb[CDB_16];       /* SCSI Cmnd Descriptor Block */
    u_int32_t   data_len;               /* length of data expected */
    u_int8_t    _resvd1;                /* reserved */
...
struct fcpio_ttmf {
    ...
    u_int8_t    lun[LUN_ADDRESS];       /* FC vNIC only: LUN address */
    u_int8_t    crn;                    /* SCSI Command Reference No. */
    u_int8_t    _resvd2[3];             /* reserved */
    u_int32_t   tmf_type;               /* task management request type */
};
...
#define FCPIO_TTMF_LUN_RESET    0x10    /* logical unit reset task management */
 ...
 */
struct fcpio_ack {
    u_int16_t  request_out;             /* last host entry received */
    u_int16_t  _resvd;
};
...
struct fcpio_fw_req {
    ...
    union {
         ...
         * Defines space needed for request
         */
        u_int8_t buf[FCPIO_FW_REQ_LEN - sizeof (struct fcpio_header)];
 ...
 */
static inline void
fcpio_color_enc(struct fcpio_fw_req *fw_req, u_int8_t color)
{
    u_int8_t *c = ((u_int8_t *) fw_req) + sizeof (struct fcpio_fw_req) - 1;
...
static inline void
fcpio_color_dec(struct fcpio_fw_req *fw_req, u_int8_t *color)
{
    u_int8_t *c = ((u_int8_t *) fw_req) + sizeof (struct fcpio_fw_req) - 1;
...
struct fcpio_lunmap_entry {
    ...
    u_int8_t    path_cnt;
    u_int16_t   flags;
    u_int16_t   update_cnt;
};

struct fcpio_lunmap_tbl {
    u_int32_t                   update_cnt;
