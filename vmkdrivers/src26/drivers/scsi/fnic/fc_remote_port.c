 ...
 * SOFTWARE.
 *
 * $Id: fc_remote_port.c 18557 2008-09-14 22:36:38Z jre $
 */
...
{
	ASSERT(TAILQ_EMPTY(&vp->vf_remote_ports));
	TAILQ_INIT(&vp->vf_remote_ports);
	if (vp->vf_rport_by_fid)
		sa_hash_destroy(vp->vf_rport_by_fid);
					     ...
					     fc_wwn_t port_name)
{
	...
	if (rp) {
		...
		if (!rp->rp_events) {
			...
			TAILQ_INSERT_TAIL(&vp->vf_remote_ports, rp, rp_list);
...
static void fc_remote_port_list(struct fc_virt_fab *vp, char *msg, 
	struct fc_remote_port *rp)
{
	SA_LOG("%s rp %6x %16llx %p",
		msg, rp->rp_fid, rp->rp_port_wwn, rp);
...
#endif /* FC_REMOTE_PORT_BY_WWPN */
	TAILQ_REMOVE(&vp->vf_remote_ports, rp, rp_list);
	fc_virt_fab_unlock(vp);
