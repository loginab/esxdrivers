 ...
 * SOFTWARE.
 *
 * $Id: fc_disc_targ.c 18557 2008-09-14 22:36:38Z jre $
 */
...
static int fcdt_gid_pn_req(struct fc_local_port *lp, struct fc_remote_port *rp);
static void fcdt_gid_pn_resp(struct fc_seq *, struct fc_frame *, void *);
static void fcdt_gid_pn_error(enum fc_event, void *);
...
static int fcdt_new_target(struct fc_local_port *lp, 
	struct fc_remote_port *rp, fc_fid_t fid, fc_wwn_t wwpn)
{
...
static void fcdt_done(struct fc_local_port *lp)
{
	...
	held = NULL;
	TAILQ_FOREACH_SAFE(rp, &vp->vf_remote_ports, rp_list, next) {
		if (next)
...
/*
 * Discover the directory information for a single target.
 * The wwpn is known, but the fid is not
 */
void fc_disc_targ_single_wwpn(struct fc_local_port *lp,
                              fc_wwn_t wwpn,
                              fc_wwn_t wwnn)
{
	struct fc_remote_port *rp;

	rp = fc_remote_port_lookup_create(lp->fl_vf, 0, wwpn, wwnn);
	if (!rp)
		fc_disc_targ_restart(lp); 	/* XXX do full discovery */
	else if (!lp->fl_disc_req)
		fcdt_gid_pn_req(lp, rp);
	else
		fc_remote_port_release(rp);
}
				...
				void *rp_arg)
{
	...
	lp = TAILQ_FIRST(&rp->rp_vf->vf_local_ports);
	ASSERT(lp);
	ASSERT(fc_frame_is_linear(fp));		/* buffer must be contiguous */
...
/*
 * Send Get ID by Portname (GID_PN) request.
 * The remote port is held by the caller for us.
 */
static int fcdt_gid_pn_req(struct fc_local_port *lp, struct fc_remote_port *rp)
{
	...
	if (fp == NULL) {
		...
		cp = fc_frame_payload_get(fp, sizeof(*cp));
		ASSERT(cp);
		fcdt_fill_dns_hdr(lp, &cp->ct, FC_NS_GID_PN, sizeof(cp->pn));
		net64_put(&cp->pn.fn_wwpn, rp->rp_port_wwn);

		ASSERT(lp->fl_dns_sess);
		ASSERT(fc_local_port_test_ready(lp));

		fc_frame_setup(fp, FC_RCTL_DD_UNSOL_CTL, FC_TYPE_CT);
		error = fc_sess_send_req(lp->fl_dns_sess, fp, fcdt_gid_pn_resp,
					fcdt_gid_pn_error, rp);
	}
	return error;
}

/*
 * Handle a response frame from Get ID by Portname (GID_PN).
 */
static void fcdt_gid_pn_resp(struct fc_seq *sp, struct fc_frame *fp,
				void *rp_arg)
{
	struct fc_remote_port *rp = rp_arg;
	struct fc_local_port *lp;
	...
	case FC_FS_ACC:
		cp = fc_frame_payload_get(fp, sizeof(*cp));
		if (cp == NULL) {
			SA_LOG("GID_PN response payload too short.  len %d",
			    fp->fr_len);
			break;
		}
		fid = net24_get(&cp->fid.fp_fid);
        fc_remote_port_set_fid(rp, fid);
        fcdt_new_target(lp, rp, rp->rp_fid, rp->rp_port_wwn);
		break;

	case FC_FS_RJT:
		if (cp->ct.ct_reason == FC_FS_RJT_UNABL &&
		    cp->ct.ct_explan == FC_FS_EXP_PNAM)
			fcdt_del_target(lp, rp);
		else
			fc_disc_targ_restart(lp);
		break;
	...
	default:
		SA_LOG("GID_PN unexpected CT response cmd %x\n", cmd);
		break;
	}
	fc_remote_port_release(rp);
	fc_frame_free(fp);
...
/*
 * Handle error from GID_PN.
 */
static void fcdt_gid_pn_error(enum fc_event event, void *rp_arg)
{
	struct fc_remote_port *rp = rp_arg;
	struct fc_local_port *lp;
