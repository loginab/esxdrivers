		  ...
		  struct lpfc_nodelist *ndlp)
{
	...
	for (i = 0; i < payload_len/sizeof(uint32_t); i++) {
		...
		lpfc_hba_put_event(phba, HBA_EVENT_RSCN, vport->fc_myDID,
				   vport->fc_myDID, fc_get_event_number(),
				   be32_to_cpu(lp[i]));
		/* Report CT event to management daemon */
		lpfc_ioctl_rscn_event(vport->phba, HBA_EVENT_RSCN);
	}
		 ...
		 struct lpfc_nodelist *ndlp)
{
	...
	if ((flag == 0) ||
		...
		printk("%s: RPS received under unexpected conditions.\n",
			__FUNCTION__);
		    ...
		    struct lpfc_iocbq *rspiocb)
{
	...
	lpfc_printf_vlog(vport, KERN_INFO, LOG_ELS,
			 "0123 FDISC completes %d reason x%08x "
			 "prevDID: x%x\n", irsp->ulpStatus,
			 irsp->un.ulpWord[4], vport->fc_prevDID);

	/*
	 * FDISCs are sent serially forcing the the a reset of the discovery
	 * timer for ALL vports waiting to send an FDISC.
	 */
	...
	if (irsp->ulpStatus) {
		...
		/* This path represents a permanent FDISC error. */
		lpfc_printf_vlog(vport, KERN_ERR, LOG_ELS,
				 "0126 FDISC failed %d reason x%08x\n",
				 irsp->ulpStatus, irsp->un.ulpWord[4]);
		goto fdisc_failed;
	}
	prsp = list_get_first(&pcmd->list, struct lpfc_dmabuf, list);
