...
static int lpfc_ioctl_found_port(struct lpfc_hba *, struct lpfc_nodelist *,
  				 void *, MAILBOX_t *, HBA_PORTATTRIBUTES *);
				  ...
				  struct lpfcCmdInput * cip, void *dataout)
{
	...
	if (found) {
		if (!lpfc_nlp_get(ndlp))
			goto err_exit;
		rc = lpfc_ioctl_found_port(phba, ndlp, dataout, pmbox, hp);
		lpfc_nlp_put(ndlp);
		kfree(pmbox);
		return rc;
	}
		      ...
		      MAILBOX_t *pmbox, HBA_PORTATTRIBUTES *hp)
{
	...
	pmbox->un.varRdRPI.reqRpi = (volatile uint16_t)ndlp->nlp_rpi;
	...
	if (psli->sli_flag & LPFC_SLI2_ACTIVE) {
		...
		pmbox->un.varRdRPI.un.sp64.tus.f.bdeSize = sizeof (struct serv_parm);
	} else {
	...
	if ((vport->fc_flag & FC_OFFLINE_MODE) ||
		...
		mbxstatus = lpfc_sli_issue_mbox_wait(phba, mboxq,
						     phba->fc_ratov * 2);

	if ((mbxstatus != MBX_SUCCESS) && (mbxstatus != MBX_TIMEOUT)) {
