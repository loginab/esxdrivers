...
{
	/* Record an HBAAPI event to the mangement interface. */
	lpfc_hba_put_event(vport->phba, HBA_EVENT_LINK_DOWN, vport->fc_myDID,
			   vport->fc_myDID, fc_get_event_number(), 0);

	/* Report CT event to management daemon */
	lpfc_ioctl_linkstate_event(vport->phba, HBA_EVENT_LINK_DOWN);
...
lpfc_linkup_port(struct lpfc_vport *vport)
{
	...
	/* Record an HBAAPI event to the mangement interface. */
	lpfc_hba_put_event(phba, HBA_EVENT_LINK_UP, phba->pport->fc_myDID,
			   phba->pport->fc_myDID, fc_get_event_number(), 0);

	/* Report CT event to management daemon */
	lpfc_ioctl_linkstate_event(vport->phba, HBA_EVENT_LINK_UP);
...
lpfc_unreg_all_rpis(struct lpfc_vport *vport)
{
	...
	if (mbox) {
		...
		if (rc != MBX_SUCCESS) {
			lpfc_printf_vlog(vport, KERN_ERR, LOG_MBOX | LOG_VPORT,
					 "1840 unreg_login failed"
                                         " (all rpis), status = 0x%06x\n", rc);
		}
		if (rc != MBX_TIMEOUT) {
			mempool_free(mbox, phba->mbox_mem_pool);
		}
...
lpfc_unreg_default_rpis(struct lpfc_vport *vport)
{
	...
	if (mbox) {
		...
		if (rc != MBX_SUCCESS) {
			lpfc_printf_vlog(vport, KERN_ERR, LOG_MBOX | LOG_VPORT,
					 "1815 Could not issue unreg_did"
                                         " (default rpis), status = 0x%06x\n", rc);
		}
		if (rc != MBX_TIMEOUT) {
			mempool_free(mbox, phba->mbox_mem_pool);
                }
