...
void lpfc_fc_nl_rcv_msg(struct sk_buff *);
#endif
...
lpfc_get_hba_model_desc(struct lpfc_hba *phba, char *mdp, char *descp)
{
	...
	if (descp && descp[0] == '\0')
		snprintf(descp, 255,
			...
			(GE) ? "GE":"Gb",
			m.bus,
			(GE) ? "FCoE CNA" : "Fibre Channel Adapter");
}
...
lpfc_pci_probe_one(struct pci_dev *pdev, const struct pci_device_id *pid)
{
	...
	/*
	 * Disable the polling logic for now.  Needs to be test more thoroughly.
	 */
	phba->cfg_poll = 0;
	phba->cfg_poll_tmo = 10;
...
lpfc_init(void)
{
...
#if !defined(__VMKLNX__)  // TODO:<lzhou> netlink APIs not implemented in vmklinux
	...
	fc_nl_sock = netlink_kernel_create(NETLINK_FCTRANSPORT, FC_NL_GROUP_CNT,
					   lpfc_fc_nl_rcv, NULL, THIS_MODULE);
#else
	fc_nl_sock = netlink_kernel_create(&init_net, NETLINK_FCTRANSPORT,
					   FC_NL_GROUP_CNT, lpfc_fc_nl_rcv_msg,
					   NULL, THIS_MODULE);
#endif
	if (!fc_nl_sock)
...
#if defined(__VMKLNX__) // TODO:<lzhou> npiv not supported right now
	if (lpfc_enable_npiv && lpfc_vport_transport_template != NULL) {
...
lpfc_exit(void)
{
	...
	security_work_q = NULL;
	if (fc_nl_sock) {
#if !defined(__VMKLNX__) // TODO:<lzhou> sock_release not implemented
		sock_release(fc_nl_sock->sk_socket);
		netlink_unregister_notifier(&lpfc_fc_netlink_notifier);
#endif /* defined(__VMKLNX__) */
		fc_nl_sock = NULL;
