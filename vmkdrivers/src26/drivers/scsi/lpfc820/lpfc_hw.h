...
typedef struct {
	...
	union {
		...
		struct lpfc_name NodeName;
		char SupportFC4Types[32];
		uint32_t SupportSpeed;
		uint32_t PortSpeed;
		uint32_t MaxFrameSize;
		char OsDeviceName[256];
		char OsNameVersion[256];
...
#define MBX_STATUS_INTERNAL_DRIVER_ERROR 252
#define MBX_STATUS_BUSY_RETRY      253
#define MBX_STATUS_TIMEOUT         254
...
typedef union {
	...
	WRITE_EVENT_LOG_VAR varWrEventLog; /* cmd = 0x39 (WRITE_EVENT_LOG) */
} MAILVARIANTS;
