...
lpfc_sli_set_dma_length(struct lpfc_hba * phba, uint32_t polling)
{
	...
	int ret = 0;
        ...
        /*
	 * The set_dma_length function restricts PCI DMA read
	 * burst sizes for a limited number of HBA models.  Don't
	 * bother processing this request for any HBA that doesn't
	 * support it.  Also set the pci_max_read configuration
	 * parameter to the actual HW value as there is no other
	 * opportunity to set it later.
	 */
        if ((FC_JEDEC_ID(phba->vpd.rev.biuRev) != PEGASUS_JEDEC_ID) &&
            (FC_JEDEC_ID(phba->vpd.rev.biuRev) != THOR_JEDEC_ID) &&
            (FC_JEDEC_ID(phba->vpd.rev.biuRev) != HELIOS_JEDEC_ID)) {
                lpfc_printf_log(phba, KERN_INFO, LOG_INIT,
                        "%d:0401 Skipping DMA length set\n",
			phba->brd_no);
	...
	case 0:
		/* The Helios, Thor, and Pegasus ASICs have the dma
 		 * read transfer size limited to 1024 by default.
 		 */
	...
	if (ret != MBX_SUCCESS) {
		if (mbox->mb.mbxStatus != MBXERR_UNKNOWN_CMD)
			lpfc_printf_log(phba, KERN_ERR, LOG_INIT,
				...
				" DMA length mbxStatus x%x data x%x\n",
				phba->brd_no, mbox->mb.mbxStatus, ret);
		else
			lpfc_printf_log(phba, KERN_ERR, LOG_INIT,
	...
	lpfc_printf_log(phba, KERN_INFO, LOG_INIT,
			"%d:0463 Set maximum DMA length to x%x Success\n",
			phba->brd_no, phba->cfg_pci_max_read);
...
failed:
	...
	if (ret == MBX_TIMEOUT) {
		...
		lpfc_printf_log(phba, KERN_ERR, LOG_INIT,
			"%d:0464 Max DMA Set failed - could not alloc memory\n",
			phba->brd_no);
		return -ENOMEM;
	}
}
...
lpfc_mbox_timeout_handler(struct lpfc_hba *phba)
{
	...
	if (pmbox == NULL) {
		lpfc_printf_log(phba, KERN_WARNING,
				LOG_MBOX | LOG_SLI,
				"0351 Active Mailbox cleared - mailbox timeout "
				"exiting\n");
		return;
	}
	...
	struct lpfc_sli_ring * pring, struct lpfc_iocbq * iocbq)
{
	...
	lpfc_ioctl_temp_event(phba, &temp_event_data);
