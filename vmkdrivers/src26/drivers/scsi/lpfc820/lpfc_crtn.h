...
int lpfc_hba_put_event(struct lpfc_hba *, uint32_t, uint32_t, uint32_t, uint32_t,
		       uint32_t);
...
struct fc_host_statistics * lpfc_get_stats(struct Scsi_Host *);
void lpfc_host_attrib_init(struct Scsi_Host *);

int lpfc_board_mode_get(struct lpfc_hba *, uint32_t *);
int lpfc_board_mode_set(struct lpfc_hba *, uint32_t, uint32_t *);
...
/* Menlo */
void lpfc_check_menlo_cfg(struct lpfc_hba *);
