...
{
	/* IEEE format 1 addresses must contain zeros in bits 59-48 */
	if (!((wwn->u.wwn[0] >> 4) == 1 &&
	      ((wwn->u.wwn[0] & 0xf) != 0 || (wwn->u.wwn[1] != 0))))
		return 1;
 ...
 * and lpfc_vport_delete to enforce a synchronous execution of vport
 * create and delete relative to discovery activities.  The
 * lpfc_vport_create routine should not return to vmkernel until it
 * can reasonably guarantee that discovery has quiesced.  This behavior
 * is required because of the timed window vmkernel allots for scsi
 * scan.  The lpfc_vport_delete routine needs the same reasonable guarantee
 * because vmkernel calls lpfc_vport_delete within 1 - 5 seconds of
 * vport_create's return to vmkernel based on its vport scsi scan policy.
 * Post FDISC LOGO, the driver must wait until its SAN teardown is
 * complete and all resources recovered before allowing vmkernel to 
 * cleanup.
 * 
...
static void lpfc_discovery_wait(struct lpfc_vport *vport)
{
	 ...
	 * fabric RA_TOV value and dev_loss tmo.  For VMware, the driver's
...
#endif
{
	...
	/* Valid vpi, store it in the vport structure. */
	vport->vpi = vpi;

	/* overide vport's log-verbose */
	if (phba->cfg_vport_log_override) {
		vport->cfg_log_verbose = phba->cfg_vport_log_override;
	}
