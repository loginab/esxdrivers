...
#define LPFC_CFG_PARAM_MAX 	64	/* Max config attributes for phba or
					   vport */
...
struct lpfc_dmabuf {
	...
	size_t     bsize;	/* amount of DMA memory allocated */
};
...
struct lpfc_timedout_iocb_ctxt {
	...
	struct lpfc_dmabuf *dmabuf0;
	struct lpfc_dmabuf *dmabuf1;
	struct lpfc_dmabuf *dmabuf2;
	struct lpfc_dmabuf *dmabuf3;

	struct lpfc_dmabufext *dmabufext0;
	struct lpfc_dmabufext *dmabufext1;
	struct lpfc_dmabufext *dmabufext2;
	struct lpfc_dmabufext *dmabufext3;
};
...
#define FC_UNLOADING		0x2	/* HBA in process of unloading drvr */
	...
	struct work_struct sc_offline_work;
        ...
        uint16_t CfgCnt;
        struct VPCfgEntry *CfgTbl[LPFC_CFG_PARAM_MAX];
...
#define LS_IGNORE_ERATT       0x4	/* intr handler should ignore ERATT */
	...
	/* Support HBAAPI parameters for RNID support. */
	uint8_t ipAddr[16];
	uint16_t ipVersion;
	uint16_t UDPport;
	...
	uint16_t CfgCnt;
	struct CfgEntry *CfgTbl[LPFC_CFG_PARAM_MAX];
	...
	uint32_t cfg_use_msi;
        uint32_t cfg_vport_log_override;
...
#define QUE_BUFTAG_BIT  (1<<31)
	...
	uint16_t hba_event_put;    /* hbaevent event put position. */
	uint16_t hba_event_get;	   /* hbaevent event get position */
	uint32_t hba_event_missed; /* hbaevent missed event count */
	HBAEVT_t hbaevt[MAX_HBAEVT]; /* Recording buffer for hbaapi event. */
};
