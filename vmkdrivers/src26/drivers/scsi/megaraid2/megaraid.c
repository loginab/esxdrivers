	...
	if(islogical) {
		...
		case MODE_SENSE: {
			...
			if (cmd->use_sg) {
				...
				for (i = 0;
				     i < cmd->use_sg && len > 0; i++, sg++) {
					buf = phys_to_virt(sg_dma_address(sg));
					memset(buf, 0, sg_dma_len(sg));
					len -= sg_dma_len(sg);
				}
			} else {
		...
		case RELEASE:
			...
			return scb;
#else
/*  In case of MEGA_HAVE_CLUSTERING disabled, Megaraid does not handle
 *  RESERVE/RELEASE. Used to result in not seeing the vmfs volumes after
 *  successful vmkfstools command run. 
 */ 
#if defined(__VMKLNX__)
                case RESERVE:   /* Fall through */
                case RELEASE:
		...
		default:
#if defined( __VMKLNX__ )
                        /* If we get here, it is most likely because the SCSI
                        * layer queued a command that we did not recognize.
                        *
                        * That being the case, it is a bad decision to return
                        * DID_BAD_TARGET here, since it will only upset the
                        * SCSI layer above. Instead, we should return a CHECK
                        * CONDITION error with sense data indicating "Invalid
                        * command operation code".
                        *
                        * Ideally, we should know up front whether there is a
                        * valid LUN here and if that is the case we should
                        * return the check condition and if no LUN exists we
                        * should return DID_BAD_TARGET.
                        *                                         --Thor
                        */
                                                                                  
                        cmd->result = (DID_OK << 16);
                        cmd->result |= (CHECK_CONDITION << 1);
                        memset(cmd->sense_buffer, 0, 14);
                        cmd->sense_buffer[0] = 0x70;    // Sense for current
                                                        // command with "valid" bit
                                                        // cleared
                        cmd->sense_buffer[2] = 0x05;    // Illegal Request
                        cmd->sense_buffer[7] = 10;      // Additional sense
                                                        // len of 10 bytes
                        cmd->sense_buffer[12] = 0x20;   // Additional sense
                                                        // code is Invalid
                                                        // command
                        cmd->sense_buffer[13] = 0x00;   // Add. sense key
                                                        //qualifier
                                                        // gives "Invalid
                                                        // Command operation
                                                        // code"
                        cmd->sense_buffer[15] = 0xc0;   // Sense key specific
                        cmd->sense_buffer[16] = 0x00;   // The error is in
                                                        //position
                        cmd->sense_buffer[17] = 0x00;   // 0 of the CDB (i.e.
                        cmd->scsi_done(cmd);
                        return NULL;
#else /* !defined(__VMKLNX__) */
			cmd->result = (DID_BAD_TARGET << 16);
...
#if defined(__VMKLNX__)
static inline int mega_vmware_chk_phys(Scsi_Cmnd * SCpnt, int islogical)
{
        /*
         * Scanning the physical channel for non-existant
         * luns != 0 causes the driver to loop mutually with
         * the linux scsi layer attempting to retry the failed
         * TUR infinitly.  For this particular case, return
         * DID_BAD_TARGET instead.  This causes the linux layer
         * to bail immeaditly
         */
        if ((SCpnt->cmnd[0] == TEST_UNIT_READY) && !islogical) {
                return((DID_BAD_TARGET << 16) | (CHECK_CONDITION << 1));
        } else {
                return((DRIVER_SENSE << 24) | (DID_OK << 16) | (CHECK_CONDITION << 1));
        }
}
#endif /* defined(__VMKLNX__) */
		...
		case 0x02:	/* ERROR_ABORTED, i.e.
			...
			if( mbox->m_out.cmd == MEGA_MBOXCMD_PASSTHRU ||
				...
				memcpy(cmd->sense_buffer, pthru->reqsensearea,
						14);
#if defined(__VMKLNX__)
                               /* TODO: <divyac> Need to verify if this
                                * applies for 2.6
                                */  
                                cmd->result = mega_vmware_chk_phys(cmd, islogical);
#else /* !defined(__VMKLNX__) */
...
#if defined(__VMKLNX__)
                /*
                 * PR192165: EVPD INQ gets status 0x01 on FW 522D or higher,
                 * default host status is BAD_TARGET, causing vmkernel scsi
                 * layer fail to create diskID. The following code makes sure
                 * it returns a faked DID_OK with a blank supported page to
                 * VPD-supported-pages INQ, so the scsi layer can create a
                 * diskID.
                 *
                 */
                case 0x01:
                        if (cmd->cmnd[0] == INQUIRY &&
                            (cmd->cmnd[1] & 1) &&       /* If EVPD bit is on */
                            (cmd->cmnd[2] == 0) ) {      /* VPD Supported page */
                            printk("EVPD INQ host status changed to DID_OK\n");
                            cmd->result |= (DID_OK << 16);
                            if (cmd->request_buffer) {
                                void *vaddr = cmd->request_buffer;
...
/*
 * TODO: <divyac> Need to verify if this workaround applies to 2.6
 */
#if !defined(__VMKLNX__)
	switch( scb->dma_type ) {
	...
	if( cmd->use_sg == 0 || (cmd->use_sg == 1 && 
		...
		if (cmd->use_sg == 0) {
			...
			length = sgl->length;
                        ...
                        scb->dma_h_bulkdata = pci_map_single(adapter->dev,
                                                cmd->request_buffer,
                                                cmd->request_bufflen,
                                                scb->dma_direction);
...
#if defined(__VMKLNX__)
        /*
         * TODO: <divyac> scsi_device_type() needs to be implemented
         */ 
...
static struct pci_device_id megaraid_pci_tbl[] = {
	...
	{PCI_VENDOR_ID_LSI_LOGIC, PCI_DEVICE_ID_PERC4_DC,
                PCI_ANY_ID, PCI_ANY_ID, 0, 0, 0},
	{PCI_VENDOR_ID_DELL, PCI_DEVICE_ID_PERC4_SE,
                PCI_ANY_ID, PCI_ANY_ID, 0, 0, 0},
