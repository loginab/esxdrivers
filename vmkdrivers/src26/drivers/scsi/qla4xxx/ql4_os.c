...
#ifdef __VMWARE__
#include <vmklinux26_scsi.h>   // For Transport type
#include "iscsi_linux.h"
#endif
...
qla4xxx_proc_dump_scanned_devices(struct scsi_qla_host *ha, struct info_str *info)
{
   ...
   list_for_each_safe(ptr, next, &ha->ddb_list) {
      ...
      copy_info(info, "    IP: %d.%d.%d.%d:%d (tpgt=%d);\n",
               ddb_entry->ip_addr[0],
               ddb_entry->ip_addr[1],
               ddb_entry->ip_addr[2],
               ddb_entry->ip_addr[3],
      ...
      copy_info(info, "    fw_state=0x%x state=0x%x flags=0x%x\n",
...
#endif
   copy_info(&info, "IP Address = %d.%d.%d.%d mask = %d.%d.%d.%d\n",
        ha->ip_address[0], ha->ip_address[1],
        ha->ip_address[2], ha->ip_address[3],
        ha->subnet_mask[0],ha->subnet_mask[1], 
        ha->subnet_mask[2], ha->subnet_mask[3]);

   copy_info(&info, "Gateway =  %d.%d.%d.%d\n",
         ha->gateway[0], ha->gateway[1],
         ha->gateway[2], ha->gateway[3]);

   copy_info(&info, "TCP Options = 0x%x\n",
        ha->tcp_options);
              ...
              enum iscsi_param param, char *buf)
{
   ...
   case ISCSI_PARAM_TPGT:
    ...
    case ISCSI_PARAM_ISID:
      len = sprintf(buf, "%u", ddb_entry->sess->sessionID);
       break;
...
#ifdef __VMWARE__
   if ((ret = iscsi_register_host(host, qla4xxx_scsi_transport)) != 0) {
      goto remove_host;
   }
#endif
...
#ifdef __VMWARE__
remove_adaptor:
   (void)iscsi_unregister_host(host, qla4xxx_scsi_transport);
#endif
...
static void __devexit qla4xxx_remove_adapter(struct pci_dev *pdev)
{
   ...
   (void)iscsi_unregister_host(ha->host, qla4xxx_scsi_transport);
