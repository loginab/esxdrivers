...
#define QLA_FW_VERSION_STR_SIZE 100
/*
...
#define MIN_LUNS		8
#define MAX_LUNS		OLD_MAX_LUNS
#define MAX_CMDS_PER_LUN	255
#define MAX_HBAS		2048 /* Used similarly as EXT_DEF_MAX_DRVR_HBAS in 3.5*/
...
#define SRB_NO_TIMER            BIT_15  /* Do not attach timer to cmd */
...
struct qla_chip_state_84xx {
	...
	uint32_t force_update;        /* Should we force update */
};
...
#define VPORT_CLEANUP_NEEDED	12
...
#define VP_DELETE_ACTIVE	29	/* VP deletion in progress */
...
#define DT_IIDMA			BIT_25
#define DT_FWI2				BIT_26
#define DT_ZIO_SUPPORTED		BIT_27
#define DT_OEM_002			BIT_28
...
#define IS_QLA24XX_TYPE(ha)  (IS_QLA24XX(ha) || IS_QLA54XX(ha) || \
                                  IS_QLA84XX(ha))
#define IS_NOT_QLA24XX_TYPE(ha) (!IS_QLA24XX(ha) && !IS_QLA54XX(ha) && \
                                   !IS_QLA84XX(ha))
...
#define IS_OEM_002(ha)		((ha)->device_type & DT_OEM_002)
...
#define MBX_UPDATE_FLASH_ACTIVE	3
	...
	spinlock_t vport_lock;		/* Virtual port synchronization */
...
#define MAX_OEM_SERIAL_LEN	16
	uint8_t		oem_serial_num[MAX_OEM_SERIAL_LEN];
...
#define QLA_LED_ALL_ON		0x07	/* yellow, green, amber. */
	...
	struct list_head	vp_del_list;	/* list of VPs to be deleted */
...
#define	IOCTL_AEN_PENDING		BIT_2
	uint8_t		*scrap_mem;	/* per ha scrap buf for ioctl usage */
	uint32_t	scrap_mem_size; /* total size */
	uint32_t	scrap_mem_used; /* portion used */
};

typedef struct fc_vport_info {
	uint32_t	free_vpids;		/* number of free VP_IDs */
	uint32_t	used_vpids;		/* number of used VP_IDs */
	uint32_t	vp_id;			/* virtual port id */
	uint32_t	vp_state;		/* virtual port state */
	uint32_t	vp_bound;		/* virtual port bound */
	uint8_t		node_name[WWN_SIZE];	/* Nodename of virtual port */
	uint8_t		port_name[WWN_SIZE];	/* Portname of virtual port */
} fc_vport_info_t;
 ...
 */
#define FLASH_IMAGE_SIZE        0x20000
...
#define CMD_SET_COMPL_STATUS(Cmnd, val) ((Cmnd)->SCp.this_residual = val)
#define CMD_SET_SCSI_STATUS(Cmnd, val) 	((Cmnd)->SCp.Status = val)
#define CMD_SET_SNSLEN(Cmnd, val)	((Cmnd)->SCp.Message = val)
