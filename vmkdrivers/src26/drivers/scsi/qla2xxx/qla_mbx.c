...
qla2x00_issue_iocb(scsi_qla_host_t *ha, void*  buffer, dma_addr_t phys_addr,
    size_t size)
{
	return qla2x00_issue_iocb_timeout(ha, buffer, phys_addr, size, MBX_TOV_SECONDS);
}
 ...
 *	size = size of buffer.
 *      tov  = timeout value for the IOCB command.
 *	TARGET_QUEUE_LOCK must be released.
 *	ADAPTER_STATE_LOCK must be released.
 *
 * Returns:
 ...
 */
int
qla2x00_issue_iocb_timeout(scsi_qla_host_t *ha, void*  buffer, dma_addr_t phys_addr,
 ...
 */
int
qla2x00_get_firmware_state(scsi_qla_host_t *ha, uint16_t *mbx)
{
	int rval;
	mbx_cmd_t mc;
	mbx_cmd_t *mcp = &mc;
    ...
    dma_addr_t buf_phys_addr, size_t buf_size, uint16_t *mb_reg)
{
	...
	mcp->out_mb = MBX_9|MBX_7|MBX_6|MBX_3|MBX_2|MBX_1|MBX_0;
	if (HAS_EXTENDED_IDS(ha)) {
		mcp->mb[1] = loop_id;
		mcp->mb[10] = data_fmt;
		mcp->out_mb |= MBX_10;
    ...
    uint16_t *orig_iocb_cnt, uint16_t *max_npiv_vports)
{
	...
	if (rval != QLA_SUCCESS) {
		...
		if (IS_FWI2_CAPABLE(ha) && max_npiv_vports) {
    ...
    struct link_statistics *stats, dma_addr_t stats_dma)
{
	...
	if (IS_FWI2_CAPABLE(ha)) {
		...
		mcp->mb[9] = ha->vp_idx;
		mcp->mb[10] = 0;
		mcp->out_mb |= MBX_10|MBX_9|MBX_4|MBX_1;
		mcp->in_mb |= MBX_1;
	} else if (HAS_EXTENDED_IDS(ha)) {
		mcp->mb[1] = loop_id;
		mcp->mb[10] = 0;
    ...
    uint16_t *port_speed, uint16_t *mb)
{
	...
	mcp->out_mb = MBX_9|MBX_5|MBX_4|MBX_3|MBX_2|MBX_1|MBX_0;
...
qla84xx_verify_chip(struct scsi_qla_host *ha, uint16_t *status)
{
	...
	do {
		...
		if (status[0] != CS_COMPLETE) {
			...
			DEBUG16(printk("%s(%ld): firmware updated to %x.\n",
			    __func__, ha->host_no,
    ...
    uint16_t *ret_mb)
{
        ...
        DEBUG11(printk("qla2x00_loopback_test: req.Options=%x iterations=%x "
...
__qla2x00_lun_reset(scsi_qla_host_t *ha, fc_port_t *fcport, uint16_t lun)
{
	...
	mcp->out_mb = MBX_3|MBX_2|MBX_1|MBX_0;
	mcp->in_mb = MBX_0;
	mcp->tov = 60;  /* VMKERNEL reduced tov by half */
	mcp->flags = 0;
	rval = qla2x00_mailbox_command(ha, mcp);
...
qla24xx_lun_reset(scsi_qla_host_t *ha, fc_port_t *fcport, uint16_t lun)
{
	...
	int_to_fcp_lun(lun, tsk->p.tsk.lun);
