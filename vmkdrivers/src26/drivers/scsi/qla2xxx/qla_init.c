...
qla24xx_pci_config(scsi_qla_host_t *ha)
{
    ...
    if (!ql2xusedefmaxrdreq && pcix_cmd_reg) {
        uint16_t pcix_cmd;
	...
	if (!ql2xusedefmaxrdreq && 
	    pci_find_capability(ha->pdev, PCI_CAP_ID_EXP))
...
qla2x00_resize_request_q(scsi_qla_host_t *ha)
{
	...
	rval = qla2x00_get_resource_cnts(ha, NULL, &orig_xchg_cnt, NULL,
	    &fw_iocb_cnt, &ha->max_npiv_vports);

	if (rval)
		return;

	ha->execution_throttle = ((ql2xexecution_throttle < orig_xchg_cnt) 
			&& (ql2xexecution_throttle) ? 
			ql2xexecution_throttle : orig_xchg_cnt);
		...
		if (rval == QLA_SUCCESS) {
			...
			if (rval == QLA_SUCCESS && ha->fw_major_version == 0) {
				...
				if ((IS_QLA24XX(ha) || IS_QLA25XX(ha) ||
				     IS_QLA84XX(ha)) && (ha->fw_attributes & BIT_2)) {
...
qla2x00_init_rings(scsi_qla_host_t *ha)
{
	...
	if (IS_QLA23XX(ha)) {
		init_cb_t *init_cb = (init_cb_t *)ha->init_cb;
		init_cb->execution_throttle = cpu_to_le16(ha->execution_throttle); 
	} else if (IS_FWI2_CAPABLE(ha)) {
		mid_init_cb->init_cb.execution_throttle = cpu_to_le16(ha->execution_throttle); 
		...
		if (rval == QLA_SUCCESS) {
			if (state[0] < FSTATE_LOSS_OF_SYNC) {
 			...
 			if (IS_QLA84XX(ha) && state[0] != FSTATE_READY) {
 			    	DEBUG16(printk("scsi(%ld): Received fw_state: "
 				    "0x%x, 84xx: %x\n", ha->host_no,
				    state[0], state[2]));
...
static inline void
qla2x00_set_model_info(scsi_qla_host_t *ha, char *model, size_t len, char *def)
{
	char *st, *en;
	uint16_t index;
...
qla2x00_rport_del(void *data)
{
	...
	if (rport){ 
		DEBUG2(printk("scsi(%ld:%d): rport_del rport %lx %x\n",
			fcport->ha->host_no, fcport->os_target_id,
...
qla2x00_reg_remote_port(scsi_qla_host_t *ha, fc_port_t *fcport)
{
	...
	DEBUG2(printk("scsi(%ld:%d) rport_add: %lx %x\n", 
		ha->host_no, fcport->os_target_id, 
		(unsigned long)rport_ids.port_name, rport_ids.port_id));
...
qla2x00_find_all_fabric_devs(scsi_qla_host_t *ha, struct list_head *new_fcports)
{
	...
	swl = kmalloc(sizeof(sw_info_t) * MAX_FIBRE_DEVICES, GFP_KERNEL);
	if (swl == NULL) {
		/*EMPTY*/
	...
	for (; loop_id <= ha->last_loop_id; loop_id++) {
		...
		if (pha->num_vhosts) {
			...
			list_for_each_entry(vha, &pha->vp_list, vp_list) {
				if (new_fcport->d_id.b24 == vha->d_id.b24) {
					found_vp = 1;
					break;
				}
			}
...
qla2x00_abort_isp(scsi_qla_host_t *ha)
{
	...
	scsi_qla_host_t *tvha, *localvha = NULL;
	unsigned long flags;
	...
	if (!status) {
		...
		spin_lock_irqsave(&ha->vport_lock, flags);
			list_for_each_entry_safe(vha, tvha, &ha->vp_list, vp_list) {
				...
				/* Get vport & temp vport references. */
				qla2xxx_vha_get(vha);
...
qla2x00_restart_isp(scsi_qla_host_t *ha)
{
	...
	if (qla2x00_isp_firmware(ha)) {
		...
		if (!(status = ha->isp_ops->chip_diag(ha))) {
			...
			if (IS_NOT_QLA24XX_TYPE(ha) && !IS_QLA25XX(ha)) {
				/*
...
qla24xx_nvram_config(scsi_qla_host_t *ha)
{
	...
	if (chksum || nv->id[0] != 'I' || nv->id[1] != 'S' || nv->id[2] != 'P'
		...
		nv->frame_payload_size = __constant_cpu_to_le16(2048);
		nv->execution_throttle = __constant_cpu_to_le16(1024);
	 ...
	 * Setup driver NVRAM options.
	 */
	qla2x00_set_model_info(ha, (char *)nv->model_name, sizeof(nv->model_name),
	    "QLA2462");
	...
	if (!ha->flags.init_done) {
#if defined(__VMKLNX__)
		if (ql2xoperationmode) {
			ha->zio_mode = QLA_ZIO_MODE_6;
			ha->zio_timer = ql2xintrdelaytimer;
		} else {
 ...
 *	
 *	Input:
 *      vha = vport adapter block pointer.
 *
 *	Returns: 
 */
int
qla24xx_configure_vhba(scsi_qla_host_t *vha)
{
	...
	rval = qla2x00_fw_ready(vha->parent);
	if (rval == QLA_SUCCESS) {
	...
	/* Login to SNS first */
	vha->isp_ops->fabric_login(vha, NPH_SNS, 0xff, 0xff, 0xfc,
	    mb, BIT_1);
	if (mb[0] != MBS_COMMAND_COMPLETE) {
		DEBUG15(qla_printk(KERN_INFO, vha,
		    "Failed SNS login: loop_id=%x mb[0]=%x mb[1]=%x "
		    "mb[2]=%x mb[6]=%x mb[7]=%x\n", NPH_SNS,
		    mb[0], mb[1], mb[2], mb[6], mb[7]));
		return (QLA_FUNCTION_FAILED);
	...
	rval = qla2x00_loop_resync(vha->parent);
