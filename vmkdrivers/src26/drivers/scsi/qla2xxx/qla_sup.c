 ...
 */
static int
qla2x00_get_oem_serial_number(scsi_qla_host_t *ha, uint32_t pcids)
{
	...
	qla2x00_read_flash_data(ha, (uint8_t *)&pci_data, pcids,
	    sizeof(pci_data));
	...
	qla2x00_read_flash_data(ha, vpd_buf, pcids +
...
static int
qla23xx_write_flash_data(struct scsi_qla_host *ha, uint8_t *buf,
    uint32_t offset, uint32_t length)
{
	int	ret = 0;
    ...
    uint32_t offset, uint32_t length)
{
	...
	/* Go with write. */
	rval = qla23xx_write_flash_data(ha, buf, offset, length);
...
/*
 * Adjust the flash write start so that it falls on the sector boundary
 * Specifically the Serdese region at 0x148000 (dword 0x52000) does not 
 * fall on sector (64K sector) boundary causing the erase to fail in turn 
 * causing write to fail
 */
void
qla24xx_adjust_flash_write_data(scsi_qla_host_t *ha,
    uint32_t faddr, uint32_t dwords, uint32_t *fnewaddr, uint32_t *newlen)
{
	uint32_t sec_mask;
	uint32_t fblock_start, fblock_end, fblock_size;

	sec_mask = 0x80000 - (ha->fdt_block_size >> 2);
	fblock_start = faddr & sec_mask;
	fblock_end = faddr + dwords + ((ha->fdt_block_size >> 2) - 1);
	fblock_end = fblock_end & sec_mask;
	fblock_size = fblock_end - fblock_start;
	*fnewaddr = fblock_start << 2;
	*newlen = fblock_size << 2;
}
    ...
    uint32_t offset, uint32_t length)
{
	...
	/* Check if the start address is at sector begining, if not
	 * adjust the start and end address and do a read-modify-write
	 */
	qla24xx_adjust_flash_write_data(ha, offset >> 2, 
	    length >> 2, &fnewaddr, &newlen);

	if (fnewaddr != offset || newlen != length ) {
		newbuf = vmalloc(newlen);	
		if (!newbuf) {
			DEBUG9(printk("%s(%ld): Unable to allocate"
			   "memory for flash write\n", __func__,
			    ha->host_no)); 
			return rval;	
		}
		qla24xx_read_flash_data(ha, (uint32_t *)newbuf, 
		    fnewaddr >> 2, newlen >> 2);
		memcpy((newbuf + (offset - fnewaddr)), buf, length);

		rval = qla24xx_write_flash_data(ha,
		    (uint32_t *)newbuf, fnewaddr >> 2, newlen >> 2);
		vfree(newbuf);
	} else {	
		rval = qla24xx_write_flash_data(ha, (uint32_t *)buf,
	    	    offset >> 2, length >> 2);
	}
...
qla2x00_get_flash_version(scsi_qla_host_t *ha, void *mbuf)
{
	...
	do {
		...
		default:
			 ...
			 */
			if (IS_OEM_002(ha) &&
			    !qla2x00_get_oem_serial_number(ha, pcids))
				break;
