...
extern void qla2x00_sp_free_dma(scsi_qla_host_t *, srb_t *);
...
qla2x00_cmd_timeout(srb_t *sp)
{
	...
	spin_lock_irqsave(&vis_ha->hardware_lock, flags);
	/* Search the command to be terminated from the outstanding
	 * command list. 
	 */
	for (cnt = 1; cnt < MAX_OUTSTANDING_COMMANDS; cnt++) {
		find_sp = vis_ha->outstanding_cmds[cnt];
		if (find_sp == sp) {
			vis_ha->outstanding_cmds[cnt] = NULL;
		}
	}
	spin_unlock_irqrestore(&vis_ha->hardware_lock, flags);
...
qla2x00_ioctl(struct scsi_device *dev, int cmd, void *arg)
{
	...
	case EXT_CC_SETINSTANCE:
		...
		if (pext->Instance < MAX_HBAS) {
			...
			spin_lock_irqsave(&((to_qla_parent(ha))->vport_lock), flags);
			qla2xxx_vha_put(ha);
			spin_unlock_irqrestore(&((to_qla_parent(ha))->vport_lock), flags);
		} else {
	...
	case INT_CC_A84_MGMT_COMMAND:
		ret = qla84xx_mgmt_command(ha, pext, mode);
		break;
	...
	case INT_CC_GET_FW_DUMP:
		ret = qla2x00_get_fw_dump(ha, pext, mode);
	break;

	case INT_CC_VF_COMMAND:
		ret = qla2x00_vf_ioctl(ha, pext);
		break;
 ...
 */
static int
qla2x00_vf_ioctl(scsi_qla_host_t *ha, EXT_IOCTL *pext)
{
       ...
       case VF_SC_VPORT_GETINFO:
               stat = qla24xx_getinfo_vport(ha, &npiv.u.vp_info, npiv.instance);
               break;
       default:
...
qla2x00_find_curr_ha(uint16_t host_inst, scsi_qla_host_t **ret_ha)
{
	...
	list_for_each(hal, &qla_hostlist) {
		...
		if (search_ha->instance == host_inst) {
			...
			if (!test_bit(VP_DELETE_ACTIVE, &search_ha->dpc_flags)) {
				qla2xxx_vha_get(search_ha);
				found++;
			}
			break;
		}
...
qla2x00_query_hba_node(scsi_qla_host_t *ha, EXT_IOCTL *pext, int mode)
{
	...
	if (ha->parent) {
		...
		ptmp_hba_node->PortCount = ha->vp_idx;
	} else {
...
/* Use port count, as only targets are counted in port_cnt */
	...
	if (IS_QLA25XX(ha)) {
		...
		if (qla2x00_get_firmware_state(ha, mbx) == QLA_SUCCESS)
...
qla2x00_query_disc_tgt(scsi_qla_host_t *ha, EXT_IOCTL *pext, int mode)
{
	...
	cnt = 0;
	 ...
	 /* Do LUN discovery. */
	qla2x00_lun_discovery(ha, tgt_fcport);
	...
	DEBUG9(printk("%s(%ld): copying data for tgt id %d. ",
	    __func__, ha->host_no, fcport->os_target_id));
...
qla2x00_get_statistics(scsi_qla_host_t *ha, EXT_IOCTL *pext, int mode)
{
	...
	if (ha->ioctl_mem_size < sizeof(struct link_statistics)) {
		if (qla2x00_get_new_ioctl_dma_mem(ha, sizeof(struct link_statistics)) !=
		    QLA_SUCCESS) {
	...
	if (IS_FWI2_CAPABLE(ha)) 
		rval = qla24xx_get_isp_stats(ha, stat_buf, stat_dma); 
	else
		rval = qla2x00_get_link_status(ha, ha->loop_id, stat_buf, stat_dma);
...
qla2x00_get_fc_statistics(scsi_qla_host_t *ha, EXT_IOCTL *pext, int mode)
{
	...
	stat_buf = ha->ioctl_mem;
	/* Send mailbox cmd to get more. */
	if ((rval = qla2x00_get_link_status(ha, fcport->loop_id, stat_buf,
			ha->ioctl_mem_phys)) != QLA_SUCCESS) {
		if (rval == BIT_0) {
			pext->Status = EXT_STATUS_NO_MEMORY;
		} else if (rval == BIT_1) {
			pext->Status = EXT_STATUS_MAILBOX;
...
qla2x00_get_tgt_lun_by_q(scsi_qla_host_t *ha, EXT_IOCTL *pext, int mode)
{
	...
	if (qla2x00_get_ioctl_scrap_mem(ha, (void **)&llist,
	    lun_data_list_size)) {
		/* not enough memory */
		pext->Status = EXT_STATUS_NO_MEMORY;
		DEBUG9_10(printk("%s(%ld): inst=%ld scrap not big enough. "
		    "size requested=%d.\n",
		    __func__, ha->host_no, ha->instance, lun_data_list_size));
		return (-ENOMEM);
	}
	...
	list_for_each_entry(fcport, &ha->fcports, list) {
		...
		list_for_each_entry(fclun, &fcport->fcluns, list) {
			...
			DEBUG9(printk("%s(%ld): lun %d enabled.\n",
			    __func__, ha->host_no, (int)fclun->lun));
		...
		DEBUG9(printk("%s(%ld): tgt %d lun count=%d.\n",
		    __func__, ha->host_no, fcport->os_target_id, entry->LunCount));
    ...
    fc_port_t *pfcport, fc_lun_t *pfclun)
{
	...
	DEBUG9(printk("%s(%ld): entered for Physical host=%ld.\n",
	    __func__, ha->host_no, pha->host_no));

	/* Assigning the done later. */
	if ((sp = qla2x00_get_new_sp(pha, pfcport, 
	    pscsi_cmd, NULL)) == NULL) {

		DEBUG9_10(printk("%s(%ld): inst=%ld ERROR cannot alloc sp.\n",
		    __func__, pha->host_no, pha->instance));
	...
	case EXT_SC_SEND_SCSI_PASSTHRU:
		...
		if (fcport == NULL) {
			...
			DEBUG9_10(printk("%s(%ld): inst=%ld received invalid "
			    "pointers. tgt_id =%d.\n",
			    __func__, ha->host_no, ha->instance, tgt_id));
    ...
    struct scsi_cmnd *pscsi_cmd, struct scsi_device *pscsi_device, int mode)
{
	...
	DEBUG9(printk("%s(%ld): inst=%ld done. host status=0x%x, "
	    "scsi status=0x%x.\n",
	    __func__, ha->host_no, ha->instance, CMD_COMPL_STATUS(pscsi_cmd),
 ...
 */
void
qla2x00_aen_flush(scsi_qla_host_t *ha)
{
	if ( (ha->ioctl->flags & IOCTL_AEN_PENDING) ) {
		ha->ioctl->flags &= ~IOCTL_AEN_PENDING;
    ...
    EXT_ELS_PT_REQ *pels_pt_req)
{
	...
	/* alloc sp */
	if ((sp = qla2x00_get_new_sp(pha, pfcport, pscsi_cmd,
	    NULL)) == NULL) {
...
#define SRB_ACTIVE_STATE        2    /* Request in Active Array */
	...
	spin_lock_irqsave(&pha->hardware_lock, cpu_flags);

	/* Get MS request packet. */
	pkt = (ms_iocb_entry_t *)qla2x00_ms_req_pkt(pha, sp);
	if (pkt == NULL) {
		/* release spin lock and return error. */
		spin_unlock_irqrestore(&pha->hardware_lock, cpu_flags);
	...
	if (IS_FWI2_CAPABLE(pha)) {
		...
		if (pels_pt_req != NULL) {
			...
			els_pkt->tx_address[0] =
			    cpu_to_le32(LSD(pha->ioctl_mem_phys));
			els_pkt->tx_address[1] =
			    cpu_to_le32(MSD(pha->ioctl_mem_phys));
			...
			ct_pkt->nport_handle =
			    cpu_to_le16(pha->mgmt_svr_loop_id);
		...
		if (pels_pt_req != NULL) {
			...
			SET_TARGET_ID(pha, pkt->loop_id, pels_pt_req->Lid);
			...
			usr_req_len = pext->RequestLen;
			usr_resp_len = pext->ResponseLen;
			SET_TARGET_ID(pha, pkt->loop_id, pha->mgmt_svr_loop_id);
		}
		 ...
		 */
		pkt->dseg_req_address[0] = cpu_to_le32(LSD(pha->ioctl_mem_phys));
		pkt->dseg_req_address[1] = cpu_to_le32(MSD(pha->ioctl_mem_phys));
		pkt->dseg_req_length = cpu_to_le32(usr_req_len);

		/* loading response payload address */
		pkt->dseg_rsp_address[0] = cpu_to_le32(LSD(pha->ioctl_mem_phys));
		pkt->dseg_rsp_address[1] =cpu_to_le32(MSD(pha->ioctl_mem_phys));
		pkt->dseg_rsp_length = cpu_to_le32(usr_resp_len);
	}

	/* set flag to indicate IOCTL MSIOCB cmd in progress */
	pha->ioctl->MSIOCB_InProgress = 1;
	...
	if (pha->ioctl->MSIOCB_InProgress == 1) {
...
qla2x00_set_led_state(scsi_qla_host_t *ha, EXT_IOCTL *pext, int mode)
{
	...
	if (IS_QLA23XX(ha)) {
		ret = qla2x00_set_led_23xx(ha, &tmp_led_state, 
		    &tmp_ext_stat, &tmp_ext_dstat);
	} else if (IS_QLA24XX(ha) || IS_QLA54XX(ha)) {
		ret = qla2x00_set_led_24xx(ha, &tmp_led_state, 
		    &tmp_ext_stat, &tmp_ext_dstat);
	} else {
		/* not supported */
    ...
    uint32_t *pext_stat, uint32_t *pext_dstat)
{
	...
	case EXT_DEF_GRN_BLINK_ON:
		...
		if(ha->isp_ops->beacon_on(ha) != QLA_SUCCESS) {
	...
	case EXT_DEF_GRN_BLINK_OFF:
		...
		if(ha->isp_ops->beacon_off(ha) != QLA_SUCCESS) {
                        DEBUG9_10(printk("%s(%ld): inst=%ld "
                                    "beacon off failed.\n",
                                    __func__, ha->host_no, ha->instance));
                                break;
...
qla2x00_add_lun(fc_port_t *fcport, uint64_t lun)
{
	...
	DEBUG3(printk("scsi(%ld): Add lun %p -- loop id=0x%04x lun=%d, "
	    "flags=%x\n", fcport->ha->host_no, fclun, fcport->loop_id,
	    fclun->lun, (int)fclun->flags));
	...
	for (retries = 3; retries; retries--) {
		if (IS_FWI2_CAPABLE(ha)) {
			...
			inq->p.cmd24.dseg_0_len =
			    __constant_cpu_to_le32(INQ_DATA_SIZE);
			if (ha->parent) {
				inq->p.cmd24.vp_index = ha->vp_idx;
			}
		} else {
			memset(inq, 0, sizeof(inq_cmd_rsp_t));
