...
qla2x00_gnn_id(scsi_qla_host_t *ha, sw_info_t *list)
{
	...
	for (i = 0; i < MAX_FIBRE_DEVICES; i++) {
		...
		/* Last device exit. */
		if (list[i].d_id.b.rsvd_1 != 0 || rval)
			break;
	}
...
{
	sprintf((char *)snn, "%s FW:v%d.%02d.%02d DVR:v%s",ha->model_number,
...
qla2x00_rsnn_nn(scsi_qla_host_t *ha)
{
	...
	ct_req->req.rsnn_nn.name_len =
	    (uint8_t)strlen((char *)ct_req->req.rsnn_nn.sym_node_name);
...
qla2x00_fdmi_rhba(scsi_qla_host_t *ha)
{
	...
	strcpy((char *)eiter->a.manufacturer, "QLogic Corporation");
	...
	if (IS_FWI2_CAPABLE(ha))
		qla2xxx_get_vpd_field(ha, "SN", eiter->a.serial_num, PAGE_SIZE);
	else {
	...
	strcpy(eiter->a.model, (char *)ha->model_number);
	...
	if (IS_FWI2_CAPABLE(ha))
		qla2xxx_get_vpd_field(ha, "MN", eiter->a.hw_version, PAGE_SIZE);
	else
		strcpy(eiter->a.hw_version, (char *)ha->adapter_id);
