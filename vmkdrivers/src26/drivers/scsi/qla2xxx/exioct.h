 ...
 *
 * Rev. 6.0     September 09, 2006
 * LC	- Added support for 4096 LUNs
 */
...
#define EXT_DEF_MAX_LUN_OD              256  /* Old driver max lun */
#define EXT_DEF_MAX_LUN_OA              2048 /* Old application max lun */
...
/*
 * To support different layer statuses in sg
 */
#define EXT_STATUS_HOST_STATUS			25
#define EXT_STATUS_DRIVER_STATUS		26
...
/*
 * Detail Status when error from HBA in sg
 */
#define EXT_DSSTATUS_DID_OK			0x00
#define EXT_DSSTATUS_DID_NO_CONNECT		0x01
#define EXT_DSSTATUS_DID_BUS_BUSY		0x02
#define EXT_DSSTATUS_DID_TIMED_OUT		0x03
#define EXT_DSSTATUS_DID_BAD_TARGET		0x04
#define EXT_DSSTATUS_DID_ABORT			0x05
#define EXT_DSSTATUS_DID_PARITY			0x06
#define EXT_DSSTATUS_DID_ERROR			0x07
#define EXT_DSSTATUS_DID_RESET			0x08
#define EXT_DSSTATUS_DID_BAD_INTR		0x09
#define EXT_DSSTATUS_DID_PASSTHROUGH		0x0A
#define EXT_DSSTATUS_DID_SOFT_ERROR		0x0B

/*
 * Detail Status when error from driver in sg
 */
#define EXT_DSSTATUS_DRIVER_OK			0x00

/*
 * Detail Status when error from driver in sg
 */
#define EXT_DSSTATUS_DRIVER_OK			0x00
#define EXT_DSSTATUS_DRIVER_BUSY		0x01
#define EXT_DSSTATUS_DRIVER_SOFT		0x02
#define EXT_DSSTATUS_DRIVER_MEDIA		0x03
#define EXT_DSSTATUS_DRIVER_ERROR		0x04
#define EXT_DSSTATUS_DRIVER_INVALID		0x05
/* Adapter driver is unable to control the SCSI bus,
   it's setting its devices offline and giving up */
#define EXT_DSSTATUS_DRIVER_TIMEOUT		0x06
#define EXT_DSSTATUS_DRIVER_HARD		0x07
/* Implies sense buffer output */
#define EXT_DSSTATUS_DRIVER_SENSE		0x08
/* Also 'or'ed with one with the following */
#define EXT_DSSTATUS_SUGGEST_RETRY		0x10
#define EXT_DSSTATUS_SUGGEST_ABORT		0x20
#define EXT_DSSTATUS_SUGGEST_REMAP		0x30
#define EXT_DSSTATUS_SUGGEST_DIE		0x40
#define EXT_DSSTATUS_SUGGEST_SENSE		0x80
...
typedef struct _EXT_HBA_NODE {
	...
	UINT8     SerialNum    [EXT_DEF_SERIAL_NUM_SIZE];	/* 128; "QFC0506A12345" */
...
typedef struct _EXT_HBA_PORT {
	...
	UINT8     PortSpeed;
	UINT16    LinkState2;                   /* Out MBX2 register, SFP information */
    	UINT16    LinkState3;                   /* Backup field not filled in at this time */
	UINT16    Unused;			/* 2 */
	UINT32    Reserved[2];			/* 12 */
...
#define	EXT_DEF_SCSI_PASSTHRU_DATA_NONE		0
...
#define	EXT_DEF_LUN_BITMASK_LIST_MAX_ENTRIES	EXT_DEF_MAX_LUNS
...
#define	EXT_DEF_LUN_COUNT          EXT_DEF_MAX_LUNS
...
#define EXTERNAL_LUN_COUNT		EXT_DEF_MAX_LUNS
#endif

#define TGT_LUN_DATA_LIST_MAX_ENTRIES       256
#define TGT_LUN_DATA_LIST_HEADER_SIZE   offsetof(TGT_LUN_DATA_LIST, DataEntry)

#define TGT_LUN_DATA_LIST_MAX_SIZE              \
    (TGT_LUN_DATA_LIST_HEADER_SIZE + \
    ((offsetof(TGT_LUN_DATA_ENTRY, Data) + lun_cnt) * \
    TGT_LUN_DATA_LIST_MAX_ENTRIES))
...
typedef struct _TGT_LUN_DATA_ENTRY {
	...
	UINT8       Data[1];	/* Variable size to support 4096 LUNs */
...
#define        VF_SC_VPORT_GETINFO     0x04
/*
...
typedef struct _VF_STRUCT {
	...
	UINT32  vf_id;
	union {        
		vport_params_t  vp_params;
		fc_vport_info_t vp_info;
	} u;
} VF_STRUCT;
