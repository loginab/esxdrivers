...
qla2x00_dev_loss_tmo_callbk(struct fc_rport *rport)
{
	...
	if (!test_bit(VP_DELETE_ACTIVE, &fcport->ha->dpc_flags))
...
qla2x00_get_host_symbolic_name(struct Scsi_Host *shost)
{
	...
	qla2x00_get_sym_node_name(ha, (uint8_t *)fc_host_symbolic_name(shost));
}
...
qla24xx_vport_delete(struct fc_vport *fc_vport)
{
	 ...
	 */				 
	spin_lock_irqsave(&pha->vport_lock, cpu_flags);
	qla2xxx_vha_put(vha);
	spin_unlock_irqrestore(&pha->vport_lock, cpu_flags);

	/* Enqueue AEN for SANSurfer to indicate vport delete. */
	if (pha->ioctl->flags & IOCTL_AEN_TRACKING_ENABLE) {
		spin_lock_irqsave(&pha->hardware_lock, cpu_flags);
		qla2x00_enqueue_aen(pha, MBA_PORT_UPDATE, NULL);
