...
#include <scsi/scsi.h> /* for scsi cmds like READ_10,... */
#include <scsi/scsi_cmnd.h> /* for scsi_cmnd */
...
#if defined(__VMKLNX__)
static int piix_check_atapi_dma(struct ata_queued_cmd *qc);
#endif /* defined(__VMKLNX__) */
...
#if defined(__VMKLNX__)
	/* 
	 * TODO: In linux 2.6.24, the field driver_data of some devices
	 * changed to ich8_2port_sata. We're still using 2.6.19.7, so 
	 * keep all of devices as ich8_sata_ahci.
	 */
...
#if defined(__VMKLNX__)
	.check_atapi_dma	= piix_check_atapi_dma,
#endif /* defined(__VMKLNX__) */
};
...
static const struct ata_port_operations ich_pata_ops = {
	...
	.port_stop		= ata_port_stop,
/* 
 * Use piix_host_stop() - have to free host private data allocated by
 * piix_init_one()
 */
#if defined(__VMKLNX__)
	.host_stop		= piix_host_stop,
...
#if defined(__VMKLNX__)
/**
 *	piix_check_atapi_dma - Check whether ATAPI DMA can be supported for this command
...
static int piix_check_atapi_dma(struct ata_queued_cmd *qc)
{
	...
	/* TODO: reddys - revisit this
	 * LITE-ON CDROMs with Intel chipsets have problems doing DMA on some
	 * commands - so possibly there may be other CDROMs as well.
	 * If ATAPI DMA is used for commands not in the following white list,
	 * ata_piix might hit the irq lost problem.
	 * So, make this check for all CDROMs!!
	 */
	switch (scsicmd[0]) {
	case READ_10:
	case WRITE_10:
