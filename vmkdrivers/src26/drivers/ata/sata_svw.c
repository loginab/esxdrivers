...
static int k2_sata_init_one (struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	probe_ent->port_flags = ATA_FLAG_SATA | ATA_FLAG_NO_LEGACY |
				ATA_FLAG_MMIO;
#if defined(__VMKLNX__)
	if (!(ent->vendor == PCI_VENDOR_ID_SERVERWORKS && 
	     (ent->device == 0x0410 || ent->device == 0x0411)))
		probe_ent->port_flags |= K2_FLAG_NO_ATAPI_DMA;

	if (probe_ent->port_flags & K2_FLAG_NO_ATAPI_DMA)
		printk("Device %04x:%04x, ATAPI DMA not supported\n",
			ent->vendor, ent->device);
	else
		printk("Device %04x:%04x, ATAPI DMA supported\n",
			ent->vendor, ent->device);
#endif /* defined(__VMKLNX__) */
...
#if defined(__VMKLNX__)
	/* HT1100 SATA controller (native SATA mode) */
	{ PCI_VDEVICE(SERVERWORKS, 0x0410), 4 },
	/* HT1100 SATA controller (PATA/IDE mode) */
	{ PCI_VDEVICE(SERVERWORKS, 0x0411), 4 },
#endif /* defined(__VMKLNX__) */
