...
/* Backported from recent kernel(~2.6.22) to handle UDMA issues with MAXTOR drives */
...
#if defined(__VMKLNX__)
	/* add missing mode filter, custom fn() needed to handle special cases */
 ...
 *     Known problematic devices so far -
 *     1. MAXTOR drives
 */

static unsigned long pdc2027x_mode_filter(const struct ata_port *ap, struct ata_device *adev, unsigned long mask)
{
	unsigned char model_num[40 /*ATA_ID_PROD_LEN*/ + 1];
	...
	ata_id_c_string(pair->id, model_num, ATA_ID_PROD_OFS,
				40 /*ATA_ID_PROD_LEN*/ + 1);
...
retry:
	 ...
	 */
	bccrl = readl(probe_ent->mmio_base + PDC_BYTE_COUNT) & 0x7fff;
	bccrh = readl(probe_ent->mmio_base + PDC_BYTE_COUNT + 0x100) & 0x7fff;
#else
...
#if defined(__VMKLNX__)
	bccrlv = readl(probe_ent->mmio_base + PDC_BYTE_COUNT) & 0x7fff;
	bccrhv = readl(probe_ent->mmio_base + PDC_BYTE_COUNT + 0x100) & 0x7fff;
#else
...
#if defined(__VMKLNX__)
	/* PLL input clock fix - description from original patch:
	 * the PLL input clock of pata_pdc2027x is sometimes detected
