...
/**
  *  access_ok - A non-operational macro that always returns 1
  *  @type: Ignored
  *  @addr: Ignored
  *  @size: Ignored
  *
  *  A non-operational macro that always returns 1
  *
  *  SYNOPSIS:
...
#if defined(__VMKLNX__)
/**
 *  get_user - Get a variable from user space
 *  @dst: variable to store result
 *  @srcPtr: source address in user space
 *
 *  This macro copies a variable from user space to kernel space. It supports
 *  simple variables and structures but not arrays. Note that sizeof(dst) can be > 
 *  sizeof(*srcPtr).
 *
 *  SYNOPSIS:
 *  #define get_user(dst, srcPtr)
 *
 *  RETURN VALUE:
 *  Returns 0 on success and 1 for failure
 */
/* _VMKLNX_CODECHECK_: get_user */
#define get_user(dst, srcPtr) \
({ \
   __typeof__(*srcPtr) _x;   \
   dst = 0; \
   copy_from_user((void *)&dst, srcPtr, sizeof(_x)); \
})
 
/* src can be an rvalue. */
/**
 * put_user - write data to a memory location in user space
 * @src: address of the data source
 * @dstPtr: destination address in the user space
 *
 * Writes the bytes from @src to @dstPtr.  The number of bytes to 
 * be written is the size of the type of @src.
 *
 * SYNOPSIS:
 *      #define put_user(src, dstPtr)
 *
 * RETURN VALUE:
 * None
 *
 ...
 * __put_user
 */
/* _VMKLNX_CODECHECK_: put_user */
#define put_user(src, dstPtr) \
({ \
   __typeof__(*dstPtr) _x = src;   \
   copy_to_user(dstPtr, &_x, sizeof(_x)); \
})
...
#if defined(__VMKLNX__)
extern unsigned long copy_to_user(void __user *to, const void *from, unsigned long len); 
extern unsigned long copy_from_user(void *to, const void __user *from, unsigned long len); 
extern unsigned long copy_in_user(void __user *to, const void __user *from, unsigned long len); 
