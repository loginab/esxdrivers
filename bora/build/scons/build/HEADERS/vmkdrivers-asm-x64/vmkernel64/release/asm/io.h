...
#if defined(__VMKLNX__)
/*
 * In vmklinux26 we no longer have a notion of a page descriptor,
 * so the pointer to the page will actually be page number.
 */
#include "vmkapi.h"
#define phys_to_page(maddr)   ((struct page *)((dma_addr_t)vmk_MachAddrToMachPage(maddr)))
/**
 *  page_to_phys - Converts a given page pointer to machine address
 *  @page: pointer to the data structure page
 *
 *  This converts a given pointer to a page to a machine address.
 *
 *  SYNOPSIS:
...
#define page_to_phys(page)    ((dma_addr_t)vmk_MachPageToMachAddr((dma_addr_t)(page)))
#else /* !defined(__VMKLNX__) */
