...
#define VMK_ERROR_CODES \
   ...
   DEFINE_VMK_ERR(VMK_VOBERR_INVALID_VOBID,     "Vob ID passed is invalid.",                  EINVAL) \
   DEFINE_VMK_ERR(VMK_VOBERR_FMT_LIMIT_EXCEEDED,"Vob format string too long.",                EFBIG) \
   DEFINE_VMK_ERR(VMK_VOBERR_INVALID_FMT_STRING,"Vob string format specifier is invalid.",    EINVAL) \
   DEFINE_VMK_ERR(VMK_VOBERR_INVALID_ATTR,      "Vob format string has invalid attribute specifiers.",   EINVAL) \
   ...
   DEFINE_VMK_ERR(VMK_FS_LOCKSTATE_IN_TRANSITION, "Lock state is in transition (ok to retry)",  EBUSY)\
   DEFINE_VMK_ERR(VMK_FS_LOCK_LOST,             "Disk lock acquired earlier, lost",           EIO)\
