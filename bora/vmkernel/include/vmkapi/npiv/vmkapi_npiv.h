...
/**
 * \brief Legacy vport arguments data structure used only VERSION 1
 * vport api implementation
 */
typedef union vport_arg {
   /** \brief argument structure for VMK_VPORT_CREATE command */
   struct {
      vmk_ScsiHost *physhost;
      struct vmk_VportData cinfo;
   } vport_create;
   
   /** \brief argument structure for VMK_VPORT_DELETE command */
   struct {
      vmk_ScsiHost *virthost;
   } vport_delete;

   /** \brief argument structure for VMK_VPORT_INFO command */
   struct {
      vmk_ScsiHost *virthost;
      struct vmk_VportInfo *info;
   } vport_getinfo;

   /** \brief argument structure for VMK_VPORT_TGT_REMOVE command */
   struct {
      vmk_ScsiHost *virthost;
      vmk_uint32 channel;
      vmk_uint32 tgtid;
